...
const HI704_SENSOR_INIT_INFO HI704_Initial_Setting_Info[] =
{
	...
	{0x01,0xf1},
	{0x01,0xf3},//PWRCTL(0x01[P0])Bit[1]:SoftwareReset.
	{0x01,0xf1},

	{0x11,0x92},//ForNoFixedFramerateBit[2]
	{0x12,0x04},//PCLKINV
	...
	{0x90,0x0c},//TIME_IN11/100_100318
	{0x91,0x0c},//TIME_OUT11/100_100318
	{0x92,0x48},
	{0x93,0x40},
	{0x98,0x38},
	{0x99,0x40},
	...
	{0xbc,0x20},
	{0xbd,0x28},
	{0xc0,0xDE},//***Change100402
	{0xc1,0xEC},//***Change100402
	{0xc2,0xDE},//***Change100402
	{0xc3,0xEC},//***Change100402
	{0xc4,0xE0},//***Change100402
	{0xc5,0xEA},//***Change100402
	{0xc6,0xE0},//***Change100402
	{0xc7,0xEa},//***Change100402
	{0xc8,0xe1},
	{0xc9,0xe8},
	{0xca,0xe1},
	{0xcb,0xe8},
	...
	//PAGE10
	{0x03,0x10},
	{0x10,0x00},//{0x10,0x03},//03,//ISPCTL1,YUVORDER(FIX)//william
	{0x11,0x43},
	{0x12,0x30},//Yoffet,dyoffseetenable
	{0x40,0x80},
	{0x41,0x00},//00DYOFS00->10_100318
	{0x48,0x90},//Contrast88->84_100318
	{0x50,0x48},//AGBRT

	{0x60,0x7f},
	{0x61,0x00},//Usedefault
	{0x62,0x80},//SATB(1.4x)
	{0x63,0x80},//SATR(1.2x)
	{0x64,0x48},//AGSAT
	{0x66,0x90},//wht_th2
	...
	{0x03,0x20},//Page20
	{0x83,0x00},//EXPNormal33.33fps
	{0x84,0xbe},
	{0x85,0x6e},
	{0x86,0x00},//EXPMin6500.00fps
	{0x87,0xfa},
	{0x88,0x02},//EXPMax8.33fps
	{0x89,0xf9},
	{0x8a,0xb8},
	{0x8B,0x3f},//EXP100
	{0x8C,0x7a},
	...
	{0x85,0x50},
	{0x86,0x25},
	{0x87,0x4d},//RMAXBDefault:50,4d
	{0x88,0x38},//RMINBDefault:3e,45-->42
	{0x89,0x3e},//BMAXBDefault:2e,2d-->30
	{0x8a,0x29},//BMINBDefault:20,22-->26-->29
	{0x8b,0x02},//OUTTH
	{0x8d,0x22},
	{0x8e,0x71},
	...
	//PAGE0
	{0x03,0x00},
	{0x01,0xf0},//0xf1->0x41:ForPreviewGreen/RedLine.
...
{	
	HI704_write_cmos_sensor(0x03, 0x00);                        //Page 0
	HI704_write_cmos_sensor(0x40, 0x01); //Hblank 344
	HI704_write_cmos_sensor(0x41, 0x58); 
	HI704_write_cmos_sensor(0x42, 0x00); //Vblank 20
...
static void HI704_Set_VGA_mode(void)
{
    ...
    HI704_write_cmos_sensor(0x83, 0x00); //EXP Normal 33.33 fps 
    HI704_write_cmos_sensor(0x84, 0xbe); 
    HI704_write_cmos_sensor(0x85, 0x6e); 
    HI704_write_cmos_sensor(0x86, 0x00); //EXPMin 6500.00 fps
    HI704_write_cmos_sensor(0x87, 0xfa); 

    HI704_write_cmos_sensor(0x8B, 0x3f); //EXP100 
    HI704_write_cmos_sensor(0x8C, 0x7a); 
    HI704_write_cmos_sensor(0x8D, 0x34); //EXP120 
    HI704_write_cmos_sensor(0x8E, 0xbc); 

    HI704_write_cmos_sensor(0x9c, 0x0b); //EXP Limit 541.67 fps 
...
static void HI704_Cal_Min_Frame_Rate(kal_uint16 min_framerate)
{
    ...
    HI704_write_cmos_sensor(0x03, 0x20);
	HI704_write_cmos_sensor(0x88, 0x02); //EXP Max 8.33 fps 
...
static void HI704_Fix_Video_Frame_Rate(kal_uint16 fix_framerate)
{
    ...
    HI704_write_cmos_sensor(0x92, 0x0f);
    HI704_write_cmos_sensor(0x93, 0xb1);
...
BOOL HI704_set_param_wb(UINT16 para)
{
    ...
    case AWB_MODE_AUTO:
		...
		HI704_write_cmos_sensor(0x85, 0x5c);
