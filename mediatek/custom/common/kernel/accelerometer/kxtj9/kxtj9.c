...
static struct platform_driver kxtj9_gsensor_driver;
...
static int kxtj9_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if((err = kxtj9_create_attr(&kxtj9_gsensor_driver.driver)))
	{
...
static int kxtj9_i2c_remove(struct i2c_client *client)
{
	...
	if((err = kxtj9_delete_attr(&kxtj9_gsensor_driver.driver)))
	{
...
/*----------------------------------------------------------------------------*/
static int kxtj9_probe(struct platform_device *pdev)
{
	struct acc_hw *hw = get_cust_acc_hw();
	GSE_FUN();

	KXTJ9_power(hw, 1);
	//kxtj9_force[0] = hw->i2c_num;
...
/*----------------------------------------------------------------------------*/
static int kxtj9_remove(struct platform_device *pdev)
{
...
static int __init kxtj9_init(void)
{
	...
	if(platform_driver_register(&kxtj9_gsensor_driver))
	{
...
{
	GSE_FUN();
	platform_driver_unregister(&kxtj9_gsensor_driver);
}
/*----------------------------------------------------------------------------*/
