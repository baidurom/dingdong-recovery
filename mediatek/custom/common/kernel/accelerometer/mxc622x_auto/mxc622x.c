...
static int mxc622x_init_flag =0; // 0<==>OK -1 <==> fail
...
#if SW_SIMULATE_I2C
////////////////////////////////////////////////////////////////////////////
#define GPIO_ACCELE_SCL_PIN GPIO184
#define GPIO_ACCELE_SDA_PIN GPIO182
...
static int mxc622x_i2c_detect(struct i2c_client *client, struct i2c_board_info *info);
...
/*----------------------------------------------------------------------------*/
static struct sensor_init_info mxc622x_init_info = {
		.name = "mxc622x",
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			{
				value = *(int *)buff_in;
				MXC622X_SetPowerMode(NULL, value);
...
//#if (!SW_SIMULATE_I2C)
	...
	obj->hw = mxc622x_get_cust_acc_hw();
	...
	obj_i2c_data = obj;
//	err=MXC622X_ReadData(NULL, databuf);
	if(MXC622X_ReadData(NULL, databuf))
	{
		printk("mxc622x probe i2c error\n");
...
{
#if (!SW_SIMULATE_I2C)
    struct acc_hw *hw = mxc622x_get_cust_acc_hw();
...
static int mxc622x_local_init(void)
{
	...
	{
		GSE_ERR("mxc622x_local_init fail.\n");
		i2c_del_driver(&mxc622x_i2c_driver);
		return -1;
	}
...
{
	GSE_FUN();
        i2c_register_board_info(0, &i2c_mxc622x, 1);
//	if(platform_driver_register(&mxc622x_gsensor_driver))
//	{
//		GSE_ERR("failed to register driver");
//		return -ENODEV;
//	}
	hwmsen_gsensor_add(&mxc622x_init_info);
