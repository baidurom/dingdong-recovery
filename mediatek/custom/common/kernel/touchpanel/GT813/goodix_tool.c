...
static int i2c_enable_commands(struct i2c_client *client, u16 addr)
{
	...
	retry = 0;
	while (i2c_master_send(client, &txbuf[0], I2C_DEVICE_ADDRESS_LEN) <
	       0) {
		retry++;
...
s32 init_wr_node(struct i2c_client *client)
{
	...
	goodix_proc_entry =
	    create_proc_entry(GOODIX_ENTRY_NAME, 0666, NULL);
...
//    memcpy(&buf[ADDR_MAX_LENGTH - cmd_head.addr_len], &cmd_head.flag_addr, cmd_head.addr_len);
	...
	for (i = 0; i < cmd_head.times; i++) {
		...
		if (true ==
		    relation(buf[ADDR_MAX_LENGTH], cmd_head.flag_val,
			     ...
			     unsigned long len, void *data)
{
	...
	if (1 == cmd_head.wr) {
		...
		copy_from_user(&cmd_head.data[ADDR_MAX_LENGTH],
			       &buff[CMD_HEAD_LENGTH], cmd_head.data_len);
		memcpy(&cmd_head.data[ADDR_MAX_LENGTH - cmd_head.addr_len],
		       cmd_head.addr, cmd_head.addr_len);

		DEBUG_ARRAY(cmd_head.data,
			    cmd_head.data_len + cmd_head.addr_len);
		DEBUG_ARRAY((u8 *) & buff[CMD_HEAD_LENGTH],
			    cmd_head.data_len);
		...
		if (tool_i2c_write
		    (&cmd_head.data[ADDR_MAX_LENGTH - cmd_head.addr_len],
		     cmd_head.data_len + cmd_head.addr_len) <= 0) {
			WARNING("[WRITE]Write data failed!\n");
			return fail;
		}

		DEBUG_ARRAY(&cmd_head.
			    data[ADDR_MAX_LENGTH - cmd_head.addr_len],
			    cmd_head.data_len + cmd_head.addr_len);
		if (cmd_head.delay) {
			    ...
			    int *eof, void *data)
{
	if (2 == cmd_head.wr) {
		...
		while (data_len > 0) {
			...
			memcpy(&page[loc], &cmd_head.data[ADDR_MAX_LENGTH],
			       len);
			loc += len;
