...
static int tpd_def_calmat_local_rotation_0[8] =
    TPD_CALIBRATION_MATRIX_ROTATION_0;
#endif
...
int i2c_read_bytes(struct i2c_client *client, u16 addr, u8 * rxbuf,
		   int len);
int i2c_write_bytes(struct i2c_client *client, u16 addr, u8 * txbuf,
		    int len);
int tpd_init_panel();
...
int i2c_enable_commands(struct i2c_client *client, u16 addr)
{
	...
	retry = 0;
	while (i2c_master_send
	       (i2c_client, &txbuf[0], I2C_DEVICE_ADDRESS_LEN) < 0) {
		retry++;
		   ...
		   int len)
{
	...
	while (left > 0) {
		if (left > MAX_I2C_MAX_TRANSFER_SIZE) {
			...
			retry = 0;
			while (i2c_master_send
			       (i2c_client, &addrBuffer[0],
				I2C_DEVICE_ADDRESS_LEN) < 0) {
				retry++;
			...
			retry = 0;
			while (i2c_master_recv
			       (i2c_client, &rxbuf[offset],
				MAX_I2C_MAX_TRANSFER_SIZE) < 0) {
				retry++;
			...
			while (i2c_master_send
				...
				if (retry == 5) {
					i2c_client->addr =
					    ...
					    addr & I2C_MASK_FLAG;
					TPD_DEBUG
					    ("I2C write 0x%X length=%d failed\n",
					     addr + offset,
					     I2C_DEVICE_ADDRESS_LEN);
					return -1;
				}
			...
			retry = 0;
			while (i2c_master_recv
			       (i2c_client, &rxbuf[offset], left) < 0) {
				retry++;
...
static void tpd_esd_check_func(struct work_struct *work)
{
	...
	if (i >= 3) {
		...
		i2c_read_bytes(i2c_client, 0x0394,
			       &esd_check_data[esd_checked_time *
					       ESD_CHECK_DATA_LEN],
			       ESD_CHECK_DATA_LEN);
		i2c_enable_commands(i2c_client, TPD_I2C_DISABLE_REG);
		if ((++esd_checked_time) >= 3) {
			if (!strncmp
			    (&esd_check_data[0],
			     &esd_check_data[ESD_CHECK_DATA_LEN],
			     ESD_CHECK_DATA_LEN)
			    && !strncmp(&esd_check_data[0],
					&esd_check_data[2 *
							ESD_CHECK_DATA_LEN],
					ESD_CHECK_DATA_LEN)) {
				force_reset_guitar();
	...
	if (tpd_halt) {
		...
		queue_delayed_work(tpd_esd_check_workqueue,
				   &tpd_esd_check_work, ESD_CHECK_CIRCLE);
	}
...
int gt827_unlocked_ioctl(struct inode *inode, struct file *filp,
...
static int touch_event_handler(void *unused)
{
	...
	do {
		...
		if (ret < 0) {
			TPD_DEBUG
			    ("[mtk-tpd] write i2c enable commands error\n");
			continue;
		}

		ret =
		    i2c_read_bytes(i2c_client, TPD_TOUCH_INFO_REG_BASE,
				   &buffer[0], TPD_TOUCH_INFO_LENGTH);
		if (ret < 0) {
			TPD_DEBUG
			    ("[mtk-tpd] i2c write communicate error during read status\n");
			continue;
		}
		...
		if (0x80 != touchBufferStatus) {
			TPD_DEBUG
			    ("[mtk-tpd] Data not ready! Touch buffer status : %x \n",
			     touchBufferStatus);
			ret =
			    i2c_enable_commands(i2c_client,
						TPD_I2C_DISABLE_REG);
			continue;
		}
		...
		ret =
		    i2c_read_bytes(i2c_client, TPD_KEY_INFO_REG_BASE,
				   &buffer[0], 1);
		if (ret < 0) {
			TPD_DEBUG
			    ("[mtk-tpd] i2c write communicate error during read key status\n");
			//continue;
		}
		...
		if (cur_mask) {
			...
			ret =
			    i2c_read_bytes(i2c_client,
					   TPD_POINT_INFO_REG_BASE, buffer,
					   finger_num *
					   TPD_POINT_INFO_LEN);
			if (ret < 0) {
				TPD_DEBUG
				    ("[mtk-tpd] i2c write communcate error during read point\n");
				continue;
			}
		} else {
			max_finger_id = 0;
		...
		for (idx = 0, valid_point = 0; idx < (max_finger_id + 1);
		     idx++) {
			u8 *ptr =
			    &buffer[valid_point * TPD_POINT_INFO_LEN];
			id = idx;
			...
			if (cur_mask & (1 << idx)) {
				...
				TPD_DEBUG
				    ("[mtk-tpd] Original position: [%4d %4d]\n",
				     x, y);
		...
		if (finger_num) {
			...
			if (tpd != NULL && tpd->dev != NULL) {
				tpd_up(x_history[lastIdx],
				       y_history[lastIdx],
				       x_history[lastIdx],
				       y_history[lastIdx], 0, lastIdx);
				input_sync(tpd->dev);
			}
		}
		...
		if (ret < 0) {
			TPD_DEBUG
			    ("[mtk-tpd] write i2c disable commands error\n");
			//continue;
		}
...
int tpd_local_init(void)
{
	...
	if ((FACTORY_BOOT == get_boot_mode())
		...
		memcpy(tpd_def_calmat, tpd_def_calmat_local_rotation_0,
		       8 * 4);
	} else {
...
void tpd_suspend(struct i2c_client *client, pm_message_t message)
{
	...
	Wrbuf[0] = 0xC0;
	ret =
	    i2c_write_bytes(i2c_client, TPD_POWER_MODE_REG, &Wrbuf[0], 1);

	if (ret < 0) {
		TPD_DEBUG
		    ("[mtk-tpd] i2c write communcate error during suspend: 0x%x\n",
		     ret);
	}
}
...
void tpd_resume(struct i2c_client *client)
{
	...
	tpd_halt = 0;
#ifdef ESD_PROTECT
	queue_delayed_work(tpd_esd_check_workqueue, &tpd_esd_check_work,
			   ESD_CHECK_CIRCLE);
#endif
}
