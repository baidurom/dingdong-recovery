...
static int gt_i2c_read_bytes(struct i2c_client *client, uint8_t * buf,
			     int len)
{
	if (i2c_read_bytes
	    (client, buf[0] << 8 | buf[1], &buf[2], len - ADDR_LENGTH)) {
		return -1;
	}
...
static int gt_i2c_write_bytes(struct i2c_client *client, uint8_t * buf,
			      int len)
{
	int ret = 1;
	if (len <= ADDR_LENGTH) {
...
static u8 get_ic_msg(struct i2c_client *client, u16 addr, u8 * msg,
		     s32 len)
{
	s32 i = 0;
...
static u8 get_ic_fw_msg(struct i2c_client *client)
{
	...
	if (fail ==
	    get_ic_msg(client,
		       READ_MSK_VER_ADDR_H << 8 | READ_MSK_VER_ADDR_L, buf,
		       4)) {
	...
	DEBUG_UPDATE("IC The mask version in rom is %c%c%c%c.\n",
		     ...
		     update_msg.ic_fw_msg.msk_ver[1],
		     update_msg.ic_fw_msg.msk_ver[2],
		     update_msg.ic_fw_msg.msk_ver[3]);
	...
	for (i = 0; i < 2; i++) {
		if (fail ==
		    get_ic_msg(client,
			       READ_FW_MSG_ADDR_H << 8 |
			       READ_FW_MSG_ADDR_L, buf, 4)) {
		...
		if (i == 0 && update_msg.force_update == 0xAA) {
			...
			DEBUG_UPDATE
			    ("If the check sum is still error,\n ");
	...
	update_msg.ic_fw_msg.version =
	    buf[ADDR_LENGTH + 2] << 8 | buf[ADDR_LENGTH + 3];
...
u8 load_update_file(struct i2c_client *client, st_fw_head * fw_head,
		    u8 * data, u8 * path)
{
	u8 mask_num = 0;
	int ret = 0;
	...
	if (path) {
		...
		for (i = 0; i < SEARCH_FILE_TIMES; i++) {
			update_msg.file =
			    filp_open(UPDATE_FILE_PATH_1, O_RDWR, 0666);
			if (IS_ERR(update_msg.file)) {
	...
	ret =
	    update_msg.file->f_op->read(update_msg.file, (char *) &buf, 6,
	...
	ret =
	    update_msg.file->f_op->read(update_msg.file, &mask_num, 1,
					&update_msg.file->f_pos);
	if (ret < 0) {
		DEBUG_UPDATE
		    ("Didn't get the mask number from the file.\n");
		goto load_failed;
	}
	...
	for (i = 0; i < mask_num; i++) {
		...
		DEBUG_UPDATE("FILE mask version:%c%c%c%c.\n",
			     fw_head->msk_ver[0], fw_head->msk_ver[1],
			     fw_head->msk_ver[2], fw_head->msk_ver[3]);
		...
		DEBUG_UPDATE("FILE chksum:0x%02x%02x%02x\n",
			     fw_head->chk_sum[0], fw_head->chk_sum[1],
			     fw_head->chk_sum[2]);

		//First two conditions
		if (is_equal
		    (fw_head->msk_ver, update_msg.ic_fw_msg.msk_ver,
		     sizeof(update_msg.ic_fw_msg.msk_ver))
		    && update_msg.ic_fw_msg.type == fw_head->type) {
			DEBUG_UPDATE
			    ("Get the same mask version and same pid.\n");
			//The third condition
			if (fw_head->version > update_msg.ic_fw_msg.version
				...
				if (is_equal
				    (fw_head->force_update, "GOODIX", 6)) {
					update_msg.gt_loc =
					    update_msg.file->f_pos -
					    FW_HEAD_LENGTH +
					    sizeof(st_fw_head) -
					    sizeof(fw_head->force_update);
				}

				ret =
				    update_msg.file->f_op->
				    read(update_msg.file, (char *)
					 data, fw_head->lenth,
					 &update_msg.file->f_pos);
				if (ret <= 0) {
					DEBUG_UPDATE
					    ("Read firmware data in file error.\n");
					goto load_failed;
				}
...
static u8 guitar_update_nvram(struct i2c_client *client,
			      st_fw_head * fw_head, u8 * nvram)
{
	int length = 0;
	int ret = 0;
	int write_bytes = 0;
	...
	st_addr =
	    (fw_head->st_addr[0] << 8) | (fw_head->st_addr[1] & 0xff);
	...
	while (length < fw_head->lenth) {
		...
		ret =
		    gt_i2c_write_bytes(client, w_buf,
				       ADDR_LENGTH + write_bytes);
		if (ret <= 0) {
			if (retry++ > 10) {
				DEBUG_UPDATE
				    ("Write the same address 10 times.Give up!\n");
				return fail;
			}
			DEBUG_UPDATE
			    ("Write error![guitar_update_nvram]\n");
			continue;
		} else {
//            DEBUG_UPDATE("w_buf:\n");
...
/*            r_buf[0] = 0x14;
			...
			for (i = 0; i < 10; i++) {
				ret =
				    gt_i2c_read_bytes(client, r_buf,
						      ADDR_LENGTH +
						      write_bytes);
				if (ret <= 0) {
					continue;
			...
			if (i >= 10) {
				DEBUG_UPDATE
				    ("Read error! Can't check the nvram data.\n");
				return fail;
			}
//            DEBUG_UPDATE("r_buf:\n");
...
static u8 guitar_update_firmware(struct i2c_client *client,
				 st_fw_head * fw_head, u8 * nvram)
{
	int retry;
	int ret;
	...
	for (retry = 0; retry < 10; retry++) {
		...
		if (!(status & 0x02)) {
			if (fail ==
			    guitar_update_nvram(client, fw_head, nvram)) {
				continue;
			} else {
				DEBUG_UPDATE
				    ("Update nvram successfully!\n");
				status |= 0x02;
				msleep(1);
			}
		}
		...
		if (1) {
			buf[0] = 0x4f;
			buf[1] = 0xf3;
			memcpy(&buf[2], fw_head->chk_sum,
			       sizeof(fw_head->chk_sum));
			ret = gt_i2c_write_bytes(client, buf, 5);
			if (ret <= 0) {
				continue;
			} else {
				DEBUG_UPDATE
				    ("Update check sum successfully!\n");
				break;
			}
		}
	}
	...
	for (retry = 0; retry < 30; retry++) {
		...
		if (fail == get_ic_msg(client, 0x00ff, buf, 1)) {
			DEBUG_UPDATE
			    ("Read address at 0x00ff error!\t retry:%d\n",
			     retry);
			continue;
		}
		...
		if (0xcc == buf[ADDR_LENGTH]) {
			...
			DEBUG_UPDATE
			    ("The value of 0x00ff: 0x%02x!\t retry:%d\n",
...
static int guitar_update_proc(void *v_cli)
{
	...
	while (retry++ < 5) {
		if (fail ==
		    guitar_update_firmware(client, &fw_head, &data[2])) {
		...
		if (buf[ADDR_LENGTH] & 0x01) {
			DEBUG_UPDATE
			    ("Check NVRCS(0x%02x) failed.(Store)\n",
			     buf[ADDR_LENGTH]);
			continue;
		}
		...
		if (buf[ADDR_LENGTH] & 0x02) {
			DEBUG_UPDATE
			    ("Check NVRCS(0x%02x) failed.(Recall)\n",
			     buf[ADDR_LENGTH]);
			continue;
		}
		...
		for (i = 0; i < 10; i++) {
			ret =
			    gt_i2c_read_bytes(client, ic_nvram,
					      ADDR_LENGTH + fw_head.lenth);
			if (ret <= 0) {
				continue;
			}
			break;
		...
		if (false ==
		    is_equal(&data[2], &ic_nvram[2], fw_head.lenth)) {
		...
		if (update_msg.gt_loc > 0) {
			...
			if (ret < 0) {
				DEBUG_UPDATE
				    ("Didn't clear the focre update flag in file.\n");
			} else {
				DEBUG_UPDATE
				    ("Clear the focre update flag in file.Location:%d, Ret:%d.\n",
				     (s32) update_msg.gt_loc, (s32) ret);
			}
		}
...
s32 init_update_proc(struct i2c_client * client)
{
	...
	if (success == update_msg.fw_flag) {
		update_msg.gt_loc = -1;
		thread =
		    kthread_run(guitar_update_proc, (void *) client,
				"guitar_update");
		if (IS_ERR(thread)) {
