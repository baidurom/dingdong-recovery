...
//liuying@wind-mobi.com 20131114 added camera proc info begin
char g_cam_proc_info[64]={0};
//liuying@wind-mobi.com 20131114 added camera proc info end
/*=============================================================================
		...
		for ( i = KDIMGSENSOR_INVOKE_DRIVER_0 ; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS ; i++ ) {
			if(DUAL_CAMERA_NONE_SENSOR != g_invokeSocketIdx[i]){
		        ...
		        if (sensorID == 0) {    //not implement this feature ID
                    ...
                    //liuying@wind-mobi.com 20131114 added camera proc info begin
                    len = strlen(g_cam_proc_info);
                    printk("len =%d\n",len);
                    sprintf(&g_cam_proc_info[len],"id=0x%04x,name=%s ",sensorID,g_invokeSensorNameStr[i]);
                    //liuying@wind-mobi.com 20131114 added camera proc info end
		            err = ERROR_NONE;
		        }
		        if(ERROR_NONE != err)
...
//liuying@wind-mobi.com 20131114 added camera proc info begin
static int  cam_proc_read(char *page, char **start, off_t off,int count, int *eof, void *data)
{
	char *p = page;
	int len=0;
	PK_DBG("Enter cam_proc_read\n");
	if(off != 0)
		return 0;
	
	//fill in camera info
    p+=sprintf(p,"%s",g_cam_proc_info);
	...
	*start = page + off;
    ...
    if (len > off)
        ...
        len = 0;
    PK_DBG("Leave cam_proc_read cnt = %d\n", len);

    return len < count ? len : count;
}

static int  cam_proc_write( struct file *file, const char *buffer, unsigned long count,
                                                                     void *data)
{
    #if 0//for future use
    char regBuf[64] = {'\0'};
...
static int __init CAMERA_HW_i2C_init(void)
{
    ...
    static struct proc_dir_entry *cam_proc_entry = NULL;
    ...
    //liuying@wind-mobi.com 20131114 added camera proc info begin
    cam_proc_entry = create_proc_entry("camera_info", 0444, NULL);
	if (cam_proc_entry == NULL) {
		PK_ERR("create_proc_entry camera_info failed\n");
	} else {
        cam_proc_entry->read_proc = cam_proc_read;
        cam_proc_entry->write_proc = cam_proc_write;
        //g_ctp_proc->owner = THIS_MODULE;
        PK_DBG("create_proc_entry camera_info success\n");
	}
    //liuying@wind-mobi.com 20131114 added camera proc info end
