...
u32 pinSet[3][8] = {
    ...
    {
        pinSet[0][IDX_PS_CMPDN+IDX_PS_ON] = GPIO_OUT_ONE;
        pinSet[0][IDX_PS_CMPDN+IDX_PS_OFF] = GPIO_OUT_ZERO;
    }
...
//        if(mt_set_gpio_out(GPIO_CAMERA_AF_EN_PIN,GPIO_OUT_ONE)){PK_DBG("[[CAMERA SENSOR] set CAMERA_POWER_PULL_PIN failed!! \n");}
	...
	if(mt_set_gpio_out(GPIO_CAMERA_AF_EN_PIN,GPIO_OUT_ONE)){PK_DBG("[[CAMERA SENSOR]  enable af en out 1 failed!! \n");}
        ...
        {
            PK_DBG("[CAMERA SENSOR] Fail to disable VCAM_A2 power\n");
            //return -EIO;
            //goto _kdCISModulePowerOn_exit_;
        }
        if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_D, mode_name)) {
            PK_DBG("[CAMERA SENSOR] Fail to OFF VCAM_D power\n");
            //return -EIO;
            //goto _kdCISModulePowerOn_exit_;
        }
        ...
        if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_D_SUB, mode_name)) {
            PK_DBG("[CAMERA SENSOR] Fail to OFF VCAM_D_SUB power\n");
            //return -EIO;
            //goto _kdCISModulePowerOn_exit_;
        }
