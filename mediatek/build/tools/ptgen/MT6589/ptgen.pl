	...
	for($index=0;$index < @arry;$index++){
		if($index == $start_replace){
			...
			for($i=0;$i < @PARTITION_FIELD;$i++){
				...
				if($PARTITION_FIELD[$i] eq "PRELOADER" ||$PARTITION_FIELD[$i] eq "DSP_BL" || $PARTITION_FIELD[$i] eq "SEC_RO"){
...
__TEMPLATE
	...
	for ($iter=0; $iter< $total_rows; $iter++){
		...
		if($PARTITION_FIELD[$iter] eq "PRELOADER" ||$PARTITION_FIELD[$iter] eq "DSP_BL" || $PARTITION_FIELD[$iter] eq "SEC_RO"){
