...
status_t SpeechVMRecorder::OpenFile()
{
    ...
    char vm_file_path[kMaxPathLength];
    memset((void *)vm_file_path, 0, kMaxPathLength);

#ifdef VM_FILENAME_ONLY_USE_VM0_TO_VM7
    char property_value[PROPERTY_VALUE_MAX];
    property_get("persist.af.modem.vm.log.index", property_value, "0");

    uint8_t vm_file_number = atoi(property_value);
    sprintf(vm_file_path, "/sdcard/mtklog/audio_dump/VMLog_%u.vm", vm_file_number++);

    sprintf(property_value, "%u", vm_file_number & 0x7);
    property_set("persist.af.modem.vm.log.index", property_value);
#else
    time_t rawtime;
    time(&rawtime);
    struct tm *timeinfo = localtime(&rawtime);
    strftime(vm_file_path, kMaxPathLength, "/sdcard/mtklog/audio_dump/VMLog_%Y_%m_%d_%H%M%S.vm", timeinfo); // TODO: use macro or const string
#endif
...
status_t SpeechVMRecorder::Close()
{
    ...
    int ret = release_wake_lock(VM_RECORD_WAKELOCK_NAME);
