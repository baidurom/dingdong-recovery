...
status_t SpeechMessengerCCCI::ReadMessage(ccci_buff_t &ccci_buff)
{
    ...
    if (read_length != sizeof(ccci_buff)) {
        ALOGW("%s() fail, read_length: %d, modem current status: %c (if case '2': false alarm)", __FUNCTION__, read_length, GetModemCurrentStatus());
        return UNKNOWN_ERROR;
    }
    return NO_ERROR;
}
...
int32_t SpeechMessengerCCCI::searchPrevAckQElement(int32_t idx)
{
    // if the queue is empty
    if (idx == iQRead) {
        ALOGD("%s() Queue empty, idx:%d, iQRead:%d", __FUNCTION__, idx, iQRead);
        return idx;
    }

    //start from idx--, which means the previous index
    idx--;
    if (idx == -1) idx += CCCI_MAX_QUEUE_NUM;

    // We can count the iQRead, which has already been sent
    int32 searchEnd = iQRead - 1;
    if (searchEnd == -1) searchEnd += CCCI_MAX_QUEUE_NUM;

    while (idx != searchEnd) {
        if (pQueue[idx].ack_type == MESSAGE_NEED_ACK) // search valid ack message only, exclude case ack = MESSAGE_CANCELED
            break;
        idx--;
        if (idx == -1) idx += CCCI_MAX_QUEUE_NUM;
    }
...
status_t SpeechMessengerCCCI::SendMessageInQueue(ccci_buff_t ccci_buff)
{
    ...
    if (count == 0) { // queue is empty
        ...
        //cancel if 'on - off - on'
        bool drop_current_message = false;
        if (count >= 2 && GetMessageID(ccci_buff) == MSG_A2M_RECORD_ON) {
            uint32 iPreAckMsg, iPrePreAckMsg;
            iPreAckMsg = searchPrevAckQElement(iQWrite);
            if (pQueue[iPreAckMsg].ack_type == MESSAGE_NEED_ACK && GetMessageID(pQueue[iPreAckMsg].ccci_buff) == MSG_A2M_RECORD_OFF) {
                ALOGD("%s(), pQueue[%d].ccci_buff.message: 0x%x", __FUNCTION__, iPreAckMsg, pQueue[iPreAckMsg].ccci_buff.message);

                iPrePreAckMsg = searchPrevAckQElement(iPreAckMsg);
                if (pQueue[iPrePreAckMsg].ack_type == MESSAGE_NEED_ACK && GetMessageID(pQueue[iPrePreAckMsg].ccci_buff) == MSG_A2M_RECORD_ON) {
                    ALOGD("%s(), pQueue[%d].ccci_buff.message: 0x%x", __FUNCTION__, iPrePreAckMsg, pQueue[iPrePreAckMsg].ccci_buff.message);

                    pQueue[iPreAckMsg].ack_type = MESSAGE_CANCELED;  // cancel previous record off
                    drop_current_message = true; // cancel current record on
                }
            }
        }

        if (drop_current_message == false) {
            pQueue[iQWrite].ccci_buff = ccci_buff;
            pQueue[iQWrite].ack_type  = ack_type;
            iQWrite++;
            if (iQWrite == CCCI_MAX_QUEUE_NUM) iQWrite -= CCCI_MAX_QUEUE_NUM;
        ...
        switch (m2a_message_id) {
            ...
            case MSG_M2A_BGSND_DATA_REQUEST: { // modem request bgs data to play
                ...
                if (pCCCI->mLad->GetApSideModemStatus(BGS_STATUS_MASK) == false) {
                    ...
                    uint32_t max_buf_length = BGS_PLAY_BUFFER_LEN;
