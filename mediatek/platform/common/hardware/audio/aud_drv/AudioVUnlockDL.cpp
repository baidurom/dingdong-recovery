...
int32 AudioVUnlockDL::GetFirstDLTime()
{
    ...
    if(mGetTime == true )
    {
        ...
        SXLOGV("[GetFirstDLTime] DL time  %d,  %d",mNewDLtime.tv_sec,mNewDLtime.tv_nsec);
        mGetTime = false;
        if(mDLtime.tv_sec != 0)
        {
...
int32_t AudioVUnlockDL::WriteStreamOutToRing(const void* buf, uint32_t datasz)
{
    ...
    {
        clock_gettime(CLOCK_MONOTONIC, &mNewDLtime);
        mNewDLtime.tv_nsec = mNewDLtime.tv_nsec + (int64_t)mStreamOutLatency*1000*1000/2 - 10000000;
...
int32_t AudioVUnlockDL::ReadRefFromRing(void*buf, uint32_t datasz, void* DLtime)
{
    ...
    SXLOGV("[ReadRefFromRing] end finaldataout %d time %d", finaldataout,mDLtime.tv_sec);
...
int32_t  AudioVUnlockDL::GetVoiceUnlockULTime( void* ULtime)
{
    ...
    *ptr= mULtime ;
    SXLOGV("ULtime->tv_sec %d, ULtime->tv_nsec %d", ptr->tv_sec,ptr->tv_nsec);
     return 0;
}
