...
onInit()
{
    ...
    IResManager* pResManager;
    //
#if CHANGE_THREAD_PRIO
    //--------------------------------------------------------------------------
    ...
    sched_p.sched_priority = PRIO_RT_CAMERA_INIT;

    if ( pthread_setschedparam(pthread_self(), SCHED_RR, &sched_p) )
    {
    ...
    pResManager = IResManager::getInstance();
    //
    if  ( pResManager != NULL )
    {
