...
status_t
CamAdapter::
setCShotState(int32_t processed_byte, int32_t free_byte)
{
    if(free_byte < 0)
    {
        MY_LOGE("can not set continuous shot free_byte as %d fps)", free_byte);
        return BAD_VALUE;
    }
    ...
    if  ( pShot != 0 )
    {
        pShot->sendCommand(eCmd_setCShotState, processed_byte, free_byte);
    }
