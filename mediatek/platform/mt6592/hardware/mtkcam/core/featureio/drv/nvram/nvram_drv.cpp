...
)
{
    ...
    case CAMERA_DATA_FLASH_AWB_CALIBRATION_DATA:
        err = readDefaultData(a_u4SensorID, a_eNvramDataType, a_pNvramData);
        if (err != NVRAM_NO_ERROR) {
            NVRAM_DRV_ERR("readDefaultData() flash AWB calibration data error:\n");
        }
        break;
    default:
        break;
...
//    static bool bCustomInit = 0; //[ALPS00424402] [CCT6589] Len shading page --> Save to NVRAM --> CCT reboot failed
    ...
    case CAMERA_DATA_FLASH_AWB_CALIBRATION_DATA:
        if (0 != GetFlashAWBCalibrationData(a_u4SensorID, (PFLASH_AWB_CALIBRATION_DATA_STRUCT)a_pNvramData))
        {
            return NVRAM_DEFAULT_DATA_READ_ERROR;
        }
        break;
