...
SetTBAToISP()     //  ISP <- PA
{
...
#if ENABLE_TSF
    MY_LOG_IF(_bEnableMyLog, "[LscMgr:%s] isTSFEnable(%d), mTSFState(%d)", __FUNCTION__, isTSFEnable(), mTSFState);

    if (m_fgUserSetTbl || (isTSFEnable() == MTRUE && mTSFState != LSCMGR_TSF_STATE_IDLE))
#else
    if (m_fgUserSetTbl)
#endif
    {
    ...
    else
    {
        ...
        MY_LOG_IF(_bEnableMyLog, "[%s] m_eLscScenario(%d), TableSize(%d), CT(%d), phyAddr(0x%08x), XSize(%d), m_eSensorOp(%d)\n",
                __FUNCTION__,
                m_eLscScenario,
