...
MRESULT AeMgr::cameraPreviewReinit()
{
    ...
    MY_LOG("[cameraPreviewReinit] m_eAECamMode:%d Real ISO:%d Exp mode: %d, Shutter:%d Sensor gain:%d Isp gain:%d frame rate:%d flare:%d %d ISO:%d\n",
...
MRESULT AeMgr::doCapFlare(MVOID *pAEStatBuf, MBOOL bIsStrobe)
{
    ...
    if(m_pIAeAlgo != NULL) {
        ...
        mCaptureMode.i2FlareGain=rAEOutput.i2FlareGain;
            ...
            if (g_rAEInitInput.rAENVRAM.rCCTConfig.bEnableCaptureThres == MFALSE) {
                mCaptureMode.i2FlareOffset = g_rAEInitInput.rAENVRAM.rCCTConfig.u4CaptureFlareOffset;
                mCaptureMode.i2FlareGain = (MINT16)(FLARE_SCALE_UNIT * FLARE_OFFSET_DOMAIN / (FLARE_OFFSET_DOMAIN  - g_rAEInitInput.rAENVRAM.rCCTConfig.u4CaptureFlareOffset));
            } else {
                mCaptureMode.i2FlareOffset=rAEOutput.i2FlareOffset;
                mCaptureMode.i2FlareGain=rAEOutput.i2FlareGain;
            }
