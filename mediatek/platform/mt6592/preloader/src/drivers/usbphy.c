...
void mt_usb_phy_recover (void)
{
	...
	//RG_USB20_INTR_CAL[4:0]	5'b01101	Internal R selection
	if (seclib_get_devinfo_with_index(19) & (0xF80))
    {
    	//print("USB HW reg: read RG_USB20_INTR_CAL 0x%x\n", USBPHY_READ8(0x06));
			USBPHY_WRITE8(0x06,((USBPHY_READ8(0x06)&0x7)|((seclib_get_devinfo_with_index(19)&0xF80)>>4)));
			print("USB HW reg: write RG_USB20_INTR_CAL 0x%x\n", USBPHY_READ8(0x06));
		}
	else
		{
			USBPHY_WRITE8(0x06,((USBPHY_READ8(0x06)&0x7)|0x68));
	...
	print("Preloader USB HW reg: offset 0x0184=0x%x\n", seclib_get_devinfo_with_index(19));

		//0x0184[3]   
		//0: keep default value of RG_USB20_ INTR_EN = 1
		//1: set RG_USB20_ INTR_EN = 0
    if (seclib_get_devinfo_with_index(19) & (0x08))
    {
		...
		if (seclib_get_devinfo_with_index(19) & (0x70))
    {
    	//print("USB HW reg: read RG_USB20_VRT_VREF_SEL 0x%x\n", USBPHY_READ8(0x05));
			USBPHY_WRITE8(0x05,((USBPHY_READ8(0x05)&0x8F)|(seclib_get_devinfo_with_index(19)&0x70)));
			print("USB HW reg: write RG_USB20_VRT_VREF_SEL 0x%x\n", USBPHY_READ8(0x05));
		}
