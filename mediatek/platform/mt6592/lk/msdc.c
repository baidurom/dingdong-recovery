...
void msdc_dump_card_status(u32 card_status)
{
    ...
    if (card_status & R1_OUT_OF_RANGE)
        dprintf(INFO,"\t[CARD_STATUS] Out of Range\n");
    if (card_status & R1_ADDRESS_ERROR)
        dprintf(INFO,"\t[CARD_STATUS] Address Error\n");
    if (card_status & R1_BLOCK_LEN_ERROR)
        dprintf(INFO,"\t[CARD_STATUS] Block Len Error\n");
    if (card_status & R1_ERASE_SEQ_ERROR)
        dprintf(INFO,"\t[CARD_STATUS] Erase Seq Error\n");
    if (card_status & R1_ERASE_PARAM)
        dprintf(INFO,"\t[CARD_STATUS] Erase Param\n");
    if (card_status & R1_WP_VIOLATION)
        dprintf(INFO,"\t[CARD_STATUS] WP Violation\n");
    if (card_status & R1_CARD_IS_LOCKED)
        dprintf(INFO,"\t[CARD_STATUS] Card is Locked\n");
    if (card_status & R1_LOCK_UNLOCK_FAILED)
        dprintf(INFO,"\t[CARD_STATUS] Lock/Unlock Failed\n");
    if (card_status & R1_COM_CRC_ERROR)
        dprintf(INFO,"\t[CARD_STATUS] Command CRC Error\n");
    if (card_status & R1_ILLEGAL_COMMAND)
        dprintf(INFO,"\t[CARD_STATUS] Illegal Command\n");
    if (card_status & R1_CARD_ECC_FAILED)
        dprintf(INFO,"\t[CARD_STATUS] Card ECC Failed\n");
    if (card_status & R1_CC_ERROR)
        dprintf(INFO,"\t[CARD_STATUS] CC Error\n");
    if (card_status & R1_ERROR)
        dprintf(INFO,"\t[CARD_STATUS] Error\n");
    if (card_status & R1_UNDERRUN)
        dprintf(INFO,"\t[CARD_STATUS] Underrun\n");
    if (card_status & R1_OVERRUN)
        dprintf(INFO,"\t[CARD_STATUS] Overrun\n");
    if (card_status & R1_CID_CSD_OVERWRITE)
        dprintf(INFO,"\t[CARD_STATUS] CID/CSD Overwrite\n");
    if (card_status & R1_WP_ERASE_SKIP)
        dprintf(INFO,"\t[CARD_STATUS] WP Eraser Skip\n");
    if (card_status & R1_CARD_ECC_DISABLED)
        dprintf(INFO,"\t[CARD_STATUS] Card ECC Disabled\n");
    if (card_status & R1_ERASE_RESET)
        dprintf(INFO,"\t[CARD_STATUS] Erase Reset\n");
    if (card_status & R1_READY_FOR_DATA)
        dprintf(INFO,"\t[CARD_STATUS] Ready for Data\n");
    if (card_status & R1_SWITCH_ERROR)
        dprintf(INFO,"\t[CARD_STATUS] Switch error\n");
    if (card_status & R1_URGENT_BKOPS)
        dprintf(INFO,"\t[CARD_STATUS] Urgent background operations\n");
    if (card_status & R1_APP_CMD)
        dprintf(INFO,"\t[CARD_STATUS] App Command\n");

    dprintf(INFO,"\t[CARD_STATUS] '%s' State\n", 
...
#if MSDC_DEBUG
    if (resp & (1 << 7))
        dprintf(INFO,"\t[OCR] Low Voltage Range\n");
    if (resp & (1 << 15))
        dprintf(INFO,"\t[OCR] 2.7-2.8 volt\n");
    if (resp & (1 << 16))
        dprintf(INFO,"\t[OCR] 2.8-2.9 volt\n");
    if (resp & (1 << 17))
        dprintf(INFO,"\t[OCR] 2.9-3.0 volt\n");
    if (resp & (1 << 18))
        dprintf(INFO,"\t[OCR] 3.0-3.1 volt\n");
    if (resp & (1 << 19))
        dprintf(INFO,"\t[OCR] 3.1-3.2 volt\n");
    if (resp & (1 << 20))
        dprintf(INFO,"\t[OCR] 3.2-3.3 volt\n");
    if (resp & (1 << 21))
        dprintf(INFO,"\t[OCR] 3.3-3.4 volt\n");
    if (resp & (1 << 22))
        dprintf(INFO,"\t[OCR] 3.4-3.5 volt\n");
    if (resp & (1 << 23))
        dprintf(INFO,"\t[OCR] 3.5-3.6 volt\n");
    if (resp & (1 << 24))
        dprintf(INFO,"\t[OCR] Switching to 1.8V Accepted (S18A)\n");
    if (resp & (1 << 30))
        dprintf(INFO,"\t[OCR] Card Capacity Status (CCS)\n");
    if (resp & (1UL << 31))
        dprintf(INFO,"\t[OCR] Card Power Up Status (Idle)\n");
    else
        dprintf(INFO,"\t[OCR] Card Power Up Status (Busy)\n");
#endif
}
...
void msdc_dump_io_resp(u32 resp)
{
    ...
    if (flags & (1 << 7))
        dprintf(INFO,"\t[IO] COM_CRC_ERR\n");
    if (flags & (1 << 6))
        dprintf(INFO,"\t[IO] Illgal command\n");   
    if (flags & (1 << 3))
        dprintf(INFO,"\t[IO] Error\n");
    if (flags & (1 << 2))
        dprintf(INFO,"\t[IO] RFU\n");
    if (flags & (1 << 1))
        dprintf(INFO,"\t[IO] Function number error\n");
    if (flags & (1 << 0))
        dprintf(INFO,"\t[IO] Out of range\n");

    dprintf(INFO,"[IO] State: %s, Data:0x%x\n", state[(resp >> 12) & 0x3], resp & 0xFF);
#endif
}
...
static void msdc_dump_dma_desc(struct mmc_host *host)
{
    ...
    if (MSG_EVT_MASK & MSG_EVT_DMA) {
        for (i = 0; i < priv->alloc_gpd; i++) {
            ...
            dprintf(INFO,"[SD%d] GD[%d]: %xh %xh %xh %xh %xh %xh %xh\n", 
        ...
        for (i = 0; i < priv->alloc_bd; i++) {
            ...
            dprintf(INFO,"[SD%d] BD[%d]: %xh %xh %xh %xh\n", 
	...
	switch(host->id){
		case 0:
            switch (mode) {
                ...
                default:
                    /* error mode! */
                    dprintf(INFO,"\t[msdc_pin_pud] host0 mode error(%d)!\n", mode);
                    break;
            }
			break;
		case 1:
            switch (mode) {
                case MSDC_PULL_NONE:
                    MSDC_SET_FIELD(MSDC1_GPIO_CMD_BASE, GPIO_R0_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_CMD_BASE, GPIO_R1_MASK, 0);
                    //sdr_set_field(MSDC0_GPIO_CMD_BASE, GPIO_PUPD_MASK, 0);
                    
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R0_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R1_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R0_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R1_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R0_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R1_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R0_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R1_MASK, 0);

                    //sdr_set_field(MSDC0_GPIO_DAT_BASE, GPIO_PUPD_MASK, 0);
                    break;
                case MSDC_PU_10K:
                    MSDC_SET_FIELD(MSDC1_GPIO_CMD_BASE, GPIO_R0_MASK, 1);
                    MSDC_SET_FIELD(MSDC1_GPIO_CMD_BASE, GPIO_R1_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_CMD_BASE, GPIO_PUPD_MASK, 0);
                    
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R0_MASK, 1);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R1_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_PUPD_MASK, 0);

                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R0_MASK, 1);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R1_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_PUPD_MASK, 0);

                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R0_MASK, 1);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R1_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_PUPD_MASK, 0);
					
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R0_MASK, 1);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R1_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_PUPD_MASK, 0);

                    /* clock pull down with 50k during card init */
                    MSDC_SET_FIELD(MSDC1_GPIO_CLK_BASE, GPIO_R0_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_CLK_BASE, GPIO_R1_MASK, 1);
                    MSDC_SET_FIELD(MSDC1_GPIO_CLK_BASE, GPIO_PUPD_MASK, 1);
                    break;
                case MSDC_PU_50K:
                    MSDC_SET_FIELD(MSDC1_GPIO_CMD_BASE, GPIO_R0_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_CMD_BASE, GPIO_R1_MASK, 1);
                    MSDC_SET_FIELD(MSDC1_GPIO_CMD_BASE, GPIO_PUPD_MASK, 0);

                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R0_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R1_MASK, 1);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_PUPD_MASK, 0);

                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R0_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R1_MASK, 1);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_PUPD_MASK, 0);

                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R0_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R1_MASK, 1);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_PUPD_MASK, 0);
					
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R0_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R1_MASK, 1);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_PUPD_MASK, 0);
                ...
                case MSDC_PD_10K:
                    MSDC_SET_FIELD(MSDC1_GPIO_CMD_BASE, GPIO_R0_MASK, 1);
                    MSDC_SET_FIELD(MSDC1_GPIO_CMD_BASE, GPIO_R1_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_CMD_BASE, GPIO_PUPD_MASK, 1);
                    
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R0_MASK, 1);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_R1_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT0_PUPD_MASK, 1);

                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R0_MASK, 1);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_R1_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT1_PUPD_MASK, 1);

                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R0_MASK, 1);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_R1_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT2_PUPD_MASK, 1);
					
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R0_MASK, 1);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_R1_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_DAT_PUPD_BASE, GPIO_DAT3_PUPD_MASK, 1);
                    break;
                case MSDC_PD_50K:
                    MSDC_SET_FIELD(MSDC1_GPIO_CMD_BASE, GPIO_R0_MASK, 0);
                    MSDC_SET_FIELD(MSDC1_GPIO_CMD_BASE, GPIO_R1_MASK, 1);
                ...
                default:
                    /* error mode! */
                    dprintf(INFO,"\t[msdc_pin_pud] host1 mode error(%d)!\n", mode);
                    break;
            }
			break;
		default:
...
static void msdc_set_driving(struct mmc_host *host,u8 clkdrv, u8 cmddrv, u8 datdrv)
{
	switch(host->id){
		case 0:
			MSDC_SET_FIELD(MSDC0_GPIO_CLK_BASE, GPIO_MSDC_DRVN, clkdrv);
			MSDC_SET_FIELD(MSDC0_GPIO_CMD_BASE, GPIO_MSDC_DRVN, cmddrv);
			MSDC_SET_FIELD(MSDC0_GPIO_DAT_BASE, GPIO_MSDC_DRVN, datdrv);
			break;
		case 1:
			MSDC_SET_FIELD(MSDC1_GPIO_CLK_BASE, GPIO_MSDC_DRVN, clkdrv);
			MSDC_SET_FIELD(MSDC1_GPIO_CMD_BASE, GPIO_MSDC_DRVN, cmddrv);
			MSDC_SET_FIELD(MSDC1_GPIO_DAT_BASE, GPIO_MSDC_DRVN, datdrv);
			break;
		case 2:
			MSDC_SET_FIELD(MSDC2_GPIO_CLK_BASE, GPIO_MSDC_DRVN, clkdrv);
			MSDC_SET_FIELD(MSDC2_GPIO_CMD_BASE, GPIO_MSDC_DRVN, cmddrv);
			MSDC_SET_FIELD(MSDC2_GPIO_DAT_BASE, GPIO_MSDC_DRVN, datdrv);
			break;
		default:
			break;
		}
...
int msdc_abort_handler(struct mmc_host *host, int abort_card)
{
	...
	while (state != 4) { // until status to "tran"
			...
			if (msdc_get_card_status(host, &status)) {
				dprintf(INFO,"Get card status failed<%d>\n", state);	
				return 1;
			}	
    ...
    if (opcode == MMC_CMD_SEND_STATUS) {
        if (SDC_IS_CMD_BUSY()) {
            ...
            if (timeout == 0) {
                ...
                dprintf(INFO,"[SD%d] CMD(%d): SDC_IS_CMD_BUSY timeout\n", 
        ...
        if (SDC_IS_BUSY()) {
            ...
            if (timeout == 0) {
                error = MMC_ERR_TIMEOUT;
                dprintf(INFO,"[SD%d] CMD(%d): SDC_IS_BUSY timeout\n", 
                    host->id, (opcode & ~(SD_CMD_BIT | SD_CMD_APP_BIT)));
                goto end;
            }
        }        
...
int msdc_wait_rsp(struct mmc_host *host, struct mmc_command *cmd)
{
    ...
    if ((status & MSDC_INT_CMDRDY) || (status & MSDC_INT_ACMDRDY) ||
        ...
        dprintf(INFO,"[SD%d] CMD(%d): RSP(%d) ERR(BADCRC)\n", 
        ...
        dprintf(INFO,"[SD%d] CMD(%d): RSP(%d) ERR(INVALID), Status:%x\n", 
    ...
    if (err == MMC_ERR_BADCRC) {
        ...
        if (tmp & 0x1800) {
            if(msdc_abort_handler(host, 1))
				dprintf(INFO,"[SD%d] abort failed\n",host->id);
        }
...
int msdc_dma_transfer(struct mmc_host *host, struct mmc_command *cmd, u8 *buf, u32 nblks)
{
    ...
    if (derr != MMC_ERR_NONE) {
        dprintf(INFO,"[SD%d] <CMD%d> DMA data error (%d)\n", host->id, cmd->opcode, derr);
		if(msdc_abort_handler(host, 1))
			dprintf(INFO,"[SD%d] abort failed\n",host->id);
    }
...
int msdc_pio_read_word(struct mmc_host *host, u32 *ptr, u32 size)
{
    ...
    while (1) {
        ...
        if (status & MSDC_INT_DATCRCERR) {
            dprintf(INFO,"[SD%d] DAT CRC error (0x%x), Left:%d/%d bytes, RXFIFO:%d\n", 
                host->id, status, size, totalsz, MSDC_RXFIFOCNT());
            err = MMC_ERR_BADCRC;
            break;
        } else if (status & MSDC_INT_DATTMO) {
            dprintf(INFO,"[SD%d] DAT TMO error (0x%x), Left: %d/%d bytes, RXFIFO:%d\n", 
                host->id, status, size, totalsz, MSDC_RXFIFOCNT());
            err = MMC_ERR_TIMEOUT;
            break;
...
int msdc_pio_write_word(struct mmc_host *host, u32 *ptr, u32 size)
{
    ...
    while (1) {
        ...
        if (status & MSDC_INT_DATCRCERR) {
            dprintf(INFO,"[SD%d] DAT CRC error (0x%x), Left DAT: %d bytes\n", 
                host->id, status, size);
            err = MMC_ERR_BADCRC;
            break;
        } else if (status & MSDC_INT_DATTMO) {
            dprintf(INFO,"[SD%d] DAT TMO error (0x%x), Left DAT: %d bytes\n", 
                host->id, status, size);
            err = MMC_ERR_TIMEOUT;
            break;
        } else if (status & MSDC_INT_XFER_COMPL) {
...
int msdc_pio_bread(struct mmc_host *host, u8 *dst, u32 src, u32 nblks)
{
    ...
    if (err != MMC_ERR_NONE) {
        if (derr != MMC_ERR_NONE) {
            dprintf(INFO,"[SD%d] Read data error (%d)\n", host->id, derr);
            if(msdc_abort_handler(host, 1))
				dprintf(INFO,"[SD%d] abort failed\n",host->id);
        } else {
            dprintf(INFO,"[SD%d] Read error (%d)\n", host->id, err);
        }
    }
...
int msdc_pio_bwrite(struct mmc_host *host, u32 dst, u8 *src, u32 nblks)
{
    ...
    if (err != MMC_ERR_NONE) {
        if (derr != MMC_ERR_NONE) {
           	dprintf(INFO,"[SD%d] Write data error (%d)\n", host->id, derr);
            if(msdc_abort_handler(host, 1))
				dprintf(INFO,"[SD%d] abort failed\n",host->id);
        } else {
            dprintf(INFO,"[SD%d] Write error (%d)\n", host->id, err);
        }
    }
    return (derr == MMC_ERR_NONE) ? err : derr;
}
...
void msdc_config_clock(struct mmc_host *host, int ddr, u32 hz)
{
    ...
    dprintf(INFO,"[SD%d] SET_CLK(%dkHz): SCLK(%dkHz) MODE(%d) DDR(%d) DIV(%d) DS(%d) RS(%d)\n",
...
int msdc_switch_volt(struct mmc_host *host, int volt)
{
    ...
    if ((MSDC_READ32(MSDC_PS) & ((1 << 24) | (0xF << 16))) == 0) {
        ...
        msdc_config_pin(host, MSDC_PIN_PULL_UP_10K);
        mdelay(5);
             ...
             do {
                for (rsmpl = 0; rsmpl < 2; rsmpl++) {
                    ...
                    /* for debugging */
                    {
                        ...
                        times++;
                        dprintf(INFO,"[SD%d] <TUNE_CMD><%d><%s> CMDRRDLY=%d, RSPL=%dh\n",
                            host->id, times, (result == MMC_ERR_NONE) ?
                            "PASS" : "FAIL", t_rrdly, t_rsmpl);
                        //printf("[SD%d] <TUNE_CMD><%d><%s> MSDC_CKGEN_RX_SDCLKO_SEL=%xh\n",
                            //host->id, times, (result == MMC_ERR_NONE) ?
                            //"PASS" : "FAIL", t_cksel);
						dprintf(INFO,"[SD%d] <TUNE_CMD><%d><%s> CMD_RSP_TA_CNTR=%xh\n",
                            host->id, times, (result == MMC_ERR_NONE) ?
                            "PASS" : "FAIL", t_cmdrtc);
                        dprintf(INFO,"[SD%d] <TUNE_CMD><%d><%s> INT_DAT_LATCH_CK_SEL=%xh\n",
                            host->id, times, (result == MMC_ERR_NONE) ?
            ...
            do {
                for (dsmpl = 0; dsmpl < 2; dsmpl++) {
					 ...
					 #if MSDC_TUNE_LOG
                    {
                        ...
                        times++;
                        dprintf(INFO,"[SD%d] <TUNE_BREAD_%d><%s> DCRC=%xh\n",
                            host->id, times, (result == MMC_ERR_NONE && dcrc == 0) ?
                            "PASS" : "FAIL", dcrc);
                        dprintf(INFO,"[SD%d] <TUNE_BREAD_%d><%s> DATRDDLY0=%xh, DATRDDLY1=%xh\n",
                            host->id, times, (result == MMC_ERR_NONE && dcrc == 0) ?
                            "PASS" : "FAIL", MSDC_READ32(MSDC_DAT_RDDLY0), MSDC_READ32(MSDC_DAT_RDDLY1));
                        //printf("[SD%d] <TUNE_BREAD_%d><%s> MSDC_CKGEN_RX_SDCLKO_SEL=%xh\n",
                            //host->id, times, (result == MMC_ERR_NONE && dcrc == 0) ?
                            //"PASS" : "FAIL", t_cksel);
                        dprintf(INFO,"[SD%d] <TUNE_BREAD_%d><%s> INT_DAT_LATCH_CK_SEL=%xh, DSMPL=%xh\n",
                            host->id, times, (result == MMC_ERR_NONE && dcrc == 0) ?
                    ...
                    do {
                        for (dsmpl = 0; dsmpl < 2; dsmpl++) {
							 ...
							 #if MSDC_TUNE_LOG
                            {
                                ...
                                dprintf(INFO,"[SD%d] <TUNE_BWRITE_%d><%s> DSPL=%d, DATWRDLY=%d\n",
                                    host->id, times, result == MMC_ERR_NONE ? "PASS" : "FAIL", 
                                    t_dspl, t_wrrdly);
								dprintf(INFO,"[SD%d] <TUNE_BWRITE_%d><%s> MSDC_DAT_RDDLY0=%xh\n",
                                    host->id, times, (result == MMC_ERR_NONE) ? "PASS" : "FAIL", 
                                    MSDC_READ32(MSDC_DAT_RDDLY0));
                               /* printf("[SD%d] <TUNE_BWRITE_%d><%s> DDR50CKD=%xh\n",
                                    host->id, times, (result == MMC_ERR_NONE) ? "PASS" : "FAIL", 
                                    t_ddrdly);*/
                                dprintf(INFO,"[SD%d] <TUNE_BWRITE_%d><%s> MSDC_PATCH_BIT1_WRDAT_CRCS=%xh\n",
                                    host->id, times, (result == MMC_ERR_NONE) ? "PASS" : "FAIL", 
                                    t_d_cntr);
...
int msdc_emmc_boot_start(struct mmc_host *host, u32 hz, int ddr, int mode, int ackdis)
{
    ...
    dprintf(INFO,"[SD%d] EMMC BOOT ACK timeout: %d ms (clkcnt: %d)\n", host->id, 
        (acktmo * 256) / (host->sclk / 1000), acktmo);
    dprintf(INFO,"[SD%d] EMMC BOOT DAT timeout: %d ms (clkcnt: %d)\n", host->id,
    ...
    if (!ackdis) {
        do {
            ...
            if (sts & EMMC_STS_BOOTACKRCV) {
                dprintf(INFO,"[SD%d] EMMC_STS(%x): boot ack received\n", host->id, sts);
                break;
            } else if (sts & EMMC_STS_BOOTACKERR) {
                dprintf(INFO,"[SD%d] EMMC_STS(%x): boot up ack error\n", host->id, sts);
                err = MMC_ERR_BADCRC;
                goto out;
            } else if (sts & EMMC_STS_BOOTACKTMO) {
                dprintf(INFO,"[SD%d] EMMC_STS(%x): boot up ack timeout\n", host->id, sts);
                err = MMC_ERR_TIMEOUT;
                goto out;
            } else if (sts & EMMC_STS_BOOTUPSTATE) {
                dprintf(INFO,"[SD%d] EMMC_STS(%x): boot up mode state\n", host->id, sts);
            } else {
                dprintf(INFO,"[SD%d] EMMC_STS(%x): boot up unexpected\n", host->id, sts);
            }
        } while (1);
    }
    ...
    do {
        ...
        if (sts & EMMC_STS_BOOTDATRCV) {
            dprintf(INFO,"[SD%d] EMMC_STS(%x): boot dat received\n", host->id, sts);
            break;
        }
...
out:
    if (err) {
        dprintf(INFO,"[SD%d] EMMC_BOOT: read boot code fail(%d), FIFOCNT=%d\n", 
...
int msdc_init(struct mmc_host *host, int id)
{
	...
	dprintf(INFO,"[%s]: set up GPIO for MSDC\n", __func__);
#endif
   ...
   // msdc_intr_unmask(host, 0x0001FF7B);
    ...
    dprintf(INFO,"[%s]: finish succesfully\n", __func__);
    return 0;
}
