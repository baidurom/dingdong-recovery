...
static void mmc_prof_card_init(void *data, u32 id, u32 counts)
{
    int err = (int)data;    
    if (!err) {
        dprintf(INFO,"[SD%d] Init Card, %d counts, %d us\n", 
...
static void mmc_prof_read(void *data, u32 id, u32 counts)
{
    ...
    dprintf(INFO,"[SD%d] Read %d bytes, %d counts, %d us, %d KB/s, Avg: %d KB/s\n", 
...
static void mmc_prof_write(void *data, u32 id, u32 counts)
{
    ...
    dprintf(INFO,"[SD%d] Write %d bytes, %d counts, %d us, %d KB/s, Avg: %d KB/s\n", 
        perf->host->id, blkcnt * blksz, counts, 
        counts * 30 + counts * 16960 / 32768, 
        blkcnt * blksz * 32 / (counts ? counts : 1),
        ((rpt->total_size / 1024) * 32768) / rpt->total_time);
    ...
    const unsigned char *buf, unsigned size)
{
    ...
    if (buf[i-1] != '\0') {
        dprintf(INFO,"SDIO: ignoring broken CISTPL_VERS_1\n");
        return 0;
    }
    ...
    const unsigned char *buf, unsigned size)
{
	...
	if (ret) {
		dprintf(INFO,"bad CISTPL_FUNCE size %u type %u\n", size, buf[0]);
        return ret;
	}
...
static void mmc_dump_tuning_blk(u8 *buf)
{
    int i;
    for (i = 0; i < 16; i++) {
        dprintf(INFO,"[TBLK%d] %x%x%x%x%x%x%x%x\n", i, 
...
static void mmc_dump_csd(struct mmc_card *card)
{
    ...
    if (mmc_card_sd(card)) {
        ...
        dprintf(INFO,"[CSD] CSD %s\n", sd_csd_ver[csd_struct]);
        dprintf(INFO,"[CSD] TACC_NS: %d ns, TACC_CLKS: %d clks\n", csd->tacc_ns, csd->tacc_clks);
        if (csd_struct == 1) {
            dprintf(INFO,"[CSD] Read/Write Blk Len = 512bytes\n");
        } else {
            dprintf(INFO,"[CSD] Read Blk Len = %d, Write Blk Len = %d\n", 
                1 << csd->read_blkbits, 1 << csd->write_blkbits);
        }
        dprintf(INFO,"[CSD] CMD Class:");
        for (i = 0; i < 12; i++) {
            if ((csd->cmdclass >> i) & 0x1)
                dprintf("'%s' ", sd_cmd_cls[i]);
        }
        dprintf(INFO,"\n");
    } else {
        csd_struct = UNSTUFF_BITS(resp, 126, 2);
        dprintf(INFO,"[CSD] CSD %s\n", mmc_csd_ver[csd_struct]);
        dprintf(INFO,"[CSD] MMCA Spec v%d\n", csd->mmca_vsn);
        dprintf(INFO,"[CSD] TACC_NS: %d ns, TACC_CLKS: %d clks\n", csd->tacc_ns, csd->tacc_clks);
        dprintf(INFO,"[CSD] Read Blk Len = %d, Write Blk Len = %d\n", 
            1 << csd->read_blkbits, 1 << csd->write_blkbits);
        dprintf(INFO,"[CSD] CMD Class:");
...
void mmc_dump_ext_csd(struct mmc_card *card)
{
    ...
    dprintf(INFO,"===========================================================\n");
    dprintf(INFO,"[EXT_CSD] EXT_CSD rev.              : v1.%d (MMCv%s)\n", 
        ext_csd[EXT_CSD_REV], rev[ext_csd[EXT_CSD_REV]]);
    dprintf(INFO,"[EXT_CSD] CSD struct rev.           : v1.%d\n", ext_csd[EXT_CSD_STRUCT]);
    dprintf(INFO,"[EXT_CSD] Supported command sets    : %xh\n", ext_csd[EXT_CSD_S_CMD_SET]);
    dprintf(INFO,"[EXT_CSD] HPI features              : %xh\n", ext_csd[EXT_CSD_HPI_FEATURE]);
    dprintf(INFO,"[EXT_CSD] BG operations support     : %xh\n", ext_csd[EXT_CSD_BKOPS_SUPP]);
    dprintf(INFO,"[EXT_CSD] BG operations status      : %xh\n", ext_csd[EXT_CSD_BKOPS_STATUS]);
    memcpy(&tmp, &ext_csd[EXT_CSD_CORRECT_PRG_SECTS_NUM], 4);
    dprintf(INFO,"[EXT_CSD] Correct prg. sectors      : %xh\n", tmp);
    dprintf(INFO,"[EXT_CSD] 1st init time after part. : %d ms\n", ext_csd[EXT_CSD_INI_TIMEOUT_AP] * 100);
    dprintf(INFO,"[EXT_CSD] Min. write perf.(DDR,52MH,8b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_DDR_W_8_52]);
    dprintf(INFO,"[EXT_CSD] Min. read perf. (DDR,52MH,8b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_DDR_R_8_52]);
    dprintf(INFO,"[EXT_CSD] TRIM timeout: %d ms\n", ext_csd[EXT_CSD_TRIM_MULT] & 0xFF * 300);
    dprintf(INFO,"[EXT_CSD] Secure feature support: %xh\n", ext_csd[EXT_CSD_SEC_FEATURE_SUPPORT]);
    dprintf(INFO,"[EXT_CSD] Secure erase timeout  : %d ms\n", 300 *
        ext_csd[EXT_CSD_ERASE_TIMEOUT_MULT] * ext_csd[EXT_CSD_SEC_ERASE_MULT]);
    dprintf(INFO,"[EXT_CSD] Secure trim timeout   : %d ms\n", 300 *
        ext_csd[EXT_CSD_ERASE_TIMEOUT_MULT] * ext_csd[EXT_CSD_SEC_TRIM_MULT]);
    dprintf(INFO,"[EXT_CSD] Access size           : %d bytes\n", ext_csd[EXT_CSD_ACC_SIZE] * 512);
    dprintf(INFO,"[EXT_CSD] HC erase unit size    : %d kbytes\n", ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE] * 512);
    dprintf(INFO,"[EXT_CSD] HC erase timeout      : %d ms\n", ext_csd[EXT_CSD_ERASE_TIMEOUT_MULT] * 300);
    dprintf(INFO,"[EXT_CSD] HC write prot grp size: %d kbytes\n", 512 *
        ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE] * ext_csd[EXT_CSD_HC_WP_GPR_SIZE]);
    dprintf(INFO,"[EXT_CSD] HC erase grp def.     : %xh\n", ext_csd[EXT_CSD_ERASE_GRP_DEF]);
    dprintf(INFO,"[EXT_CSD] Reliable write sect count: %xh\n", ext_csd[EXT_CSD_REL_WR_SEC_C]);
    dprintf(INFO,"[EXT_CSD] Sleep current (VCC) : %xh\n", ext_csd[EXT_CSD_S_C_VCC]);
    dprintf(INFO,"[EXT_CSD] Sleep current (VCCQ): %xh\n", ext_csd[EXT_CSD_S_C_VCCQ]);
    dprintf(INFO,"[EXT_CSD] Sleep/awake timeout : %d ns\n", 
        100 * (2 << ext_csd[EXT_CSD_S_A_TIMEOUT]));
    memcpy(&tmp, &ext_csd[EXT_CSD_SEC_CNT], 4);
    dprintf(INFO,"[EXT_CSD] Sector count : %xh\n", tmp);
    dprintf(INFO,"[EXT_CSD] Min. WR Perf.  (52MH,8b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_W_8_52]);
    dprintf(INFO,"[EXT_CSD] Min. Read Perf.(52MH,8b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_R_8_52]);
    dprintf(INFO,"[EXT_CSD] Min. WR Perf.  (26MH,8b,52MH,4b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_W_8_26_4_25]);
    dprintf(INFO,"[EXT_CSD] Min. Read Perf.(26MH,8b,52MH,4b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_R_8_26_4_25]);
    dprintf(INFO,"[EXT_CSD] Min. WR Perf.  (26MH,4b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_W_4_26]);
    dprintf(INFO,"[EXT_CSD] Min. Read Perf.(26MH,4b): %xh\n", ext_csd[EXT_CSD_MIN_PERF_R_4_26]);
    dprintf(INFO,"[EXT_CSD] Power class: %x\n", ext_csd[EXT_CSD_PWR_CLASS]);
    dprintf(INFO,"[EXT_CSD] Power class(DDR,52MH,3.6V): %xh\n", ext_csd[EXT_CSD_PWR_CL_DDR_52_360]);
    dprintf(INFO,"[EXT_CSD] Power class(DDR,52MH,1.9V): %xh\n", ext_csd[EXT_CSD_PWR_CL_DDR_52_195]);
    dprintf(INFO,"[EXT_CSD] Power class(26MH,3.6V)    : %xh\n", ext_csd[EXT_CSD_PWR_CL_26_360]);
    dprintf(INFO,"[EXT_CSD] Power class(52MH,3.6V)    : %xh\n", ext_csd[EXT_CSD_PWR_CL_52_360]);    
    dprintf(INFO,"[EXT_CSD] Power class(26MH,1.9V)    : %xh\n", ext_csd[EXT_CSD_PWR_CL_26_195]);
    dprintf(INFO,"[EXT_CSD] Power class(52MH,1.9V)    : %xh\n", ext_csd[EXT_CSD_PWR_CL_52_195]);
    dprintf(INFO,"[EXT_CSD] Part. switch timing    : %xh\n", ext_csd[EXT_CSD_PART_SWITCH_TIME]);
    dprintf(INFO,"[EXT_CSD] Out-of-INTR busy timing: %xh\n", ext_csd[EXT_CSD_OUT_OF_INTR_TIME]);  
    dprintf(INFO,"[EXT_CSD] Card type       : %xh\n", ext_csd[EXT_CSD_CARD_TYPE]);
    dprintf(INFO,"[EXT_CSD] Command set     : %xh\n", ext_csd[EXT_CSD_CMD_SET]);
    dprintf(INFO,"[EXT_CSD] Command set rev.: %xh\n", ext_csd[EXT_CSD_CMD_SET_REV]);
    dprintf(INFO,"[EXT_CSD] HS timing       : %xh\n", ext_csd[EXT_CSD_HS_TIMING]);
    dprintf(INFO,"[EXT_CSD] Bus width       : %xh\n", ext_csd[EXT_CSD_BUS_WIDTH]);
    dprintf(INFO,"[EXT_CSD] Erase memory content : %xh\n", ext_csd[EXT_CSD_ERASED_MEM_CONT]);
    dprintf(INFO,"[EXT_CSD] Partition config      : %xh\n", ext_csd[EXT_CSD_PART_CFG]);
    dprintf(INFO,"[EXT_CSD] Boot partition size   : %d kbytes\n", ext_csd[EXT_CSD_BOOT_SIZE_MULT] * 128);    
    dprintf(INFO,"[EXT_CSD] Boot information      : %xh\n", ext_csd[EXT_CSD_BOOT_INFO]);    
    dprintf(INFO,"[EXT_CSD] Boot config protection: %xh\n", ext_csd[EXT_CSD_BOOT_CONFIG_PROT]);
    dprintf(INFO,"[EXT_CSD] Boot bus width        : %xh\n", ext_csd[EXT_CSD_BOOT_BUS_WIDTH]);
    dprintf(INFO,"[EXT_CSD] Boot area write prot  : %xh\n", ext_csd[EXT_CSD_BOOT_WP]);
    dprintf(INFO,"[EXT_CSD] User area write prot  : %xh\n", ext_csd[EXT_CSD_USR_WP]);
    dprintf(INFO,"[EXT_CSD] FW configuration      : %xh\n", ext_csd[EXT_CSD_FW_CONFIG]);
    dprintf(INFO,"[EXT_CSD] RPMB size : %d kbytes\n", ext_csd[EXT_CSD_RPMB_SIZE_MULT] * 128);
    dprintf(INFO,"[EXT_CSD] Write rel. setting  : %xh\n", ext_csd[EXT_CSD_WR_REL_SET]);
    dprintf(INFO,"[EXT_CSD] Write rel. parameter: %xh\n", ext_csd[EXT_CSD_WR_REL_PARAM]);
    dprintf(INFO,"[EXT_CSD] Start background ops : %xh\n", ext_csd[EXT_CSD_BKOPS_START]);
    dprintf(INFO,"[EXT_CSD] Enable background ops: %xh\n", ext_csd[EXT_CSD_BKOPS_EN]);
    dprintf(INFO,"[EXT_CSD] H/W reset function   : %xh\n", ext_csd[EXT_CSD_RST_N_FUNC]);
    dprintf(INFO,"[EXT_CSD] HPI management       : %xh\n", ext_csd[EXT_CSD_HPI_MGMT]);
    memcpy(&tmp, &ext_csd[EXT_CSD_MAX_ENH_SIZE_MULT], 4);
    dprintf(INFO,"[EXT_CSD] Max. enhanced area size : %xh (%d kbytes)\n", 
        tmp & 0x00FFFFFF, (tmp & 0x00FFFFFF) * 512 *
        ext_csd[EXT_CSD_HC_WP_GPR_SIZE] * ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE]);
    dprintf(INFO,"[EXT_CSD] Part. support  : %xh\n", ext_csd[EXT_CSD_PART_SUPPORT]);
    dprintf(INFO,"[EXT_CSD] Part. attribute: %xh\n", ext_csd[EXT_CSD_PART_ATTR]);
    dprintf(INFO,"[EXT_CSD] Part. setting  : %xh\n", ext_csd[EXT_CSD_PART_SET_COMPL]);
    dprintf(INFO,"[EXT_CSD] General purpose 1 size : %xh (%d kbytes)\n", 
    ...
    dprintf(INFO,"[EXT_CSD] General purpose 2 size : %xh (%d kbytes)\n", 
        ...
        (ext_csd[EXT_CSD_GP2_SIZE_MULT + 0] | 
         ...
         ext_csd[EXT_CSD_HC_WP_GPR_SIZE] * 
         ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE]);
    dprintf(INFO,"[EXT_CSD] General purpose 3 size : %xh (%d kbytes)\n", 
        ...
        (ext_csd[EXT_CSD_GP3_SIZE_MULT + 0] | 
         ...
         ext_csd[EXT_CSD_HC_WP_GPR_SIZE] * 
         ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE]);
    dprintf(INFO,"[EXT_CSD] General purpose 4 size : %xh (%d kbytes)\n", 
        ...
        (ext_csd[EXT_CSD_GP4_SIZE_MULT + 0] | 
         ...
         ext_csd[EXT_CSD_HC_WP_GPR_SIZE] * 
         ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE]);
    dprintf(INFO,"[EXT_CSD] Enh. user area size : %xh (%d kbytes)\n", 
        ...
        (ext_csd[EXT_CSD_ENH_SIZE_MULT + 0] | 
         ...
         ext_csd[EXT_CSD_HC_WP_GPR_SIZE] * 
         ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE]);
    dprintf(INFO,"[EXT_CSD] Enh. user area start: %xh\n", 
        (ext_csd[EXT_CSD_ENH_START_ADDR + 0] |
         ext_csd[EXT_CSD_ENH_START_ADDR + 1] << 8 |
         ext_csd[EXT_CSD_ENH_START_ADDR + 2] << 16 |
         ext_csd[EXT_CSD_ENH_START_ADDR + 3]) << 24);
    dprintf(INFO,"[EXT_CSD] Bad block mgmt mode: %xh\n", ext_csd[EXT_CSD_BADBLK_MGMT]);
    dprintf(INFO,"===========================================================\n");
}
#endif
...
int mmc_addr_trans_tbl_init(void)
{
    ...
    if (mmc_card_mmc(card) && ext_csd[EXT_CSD_REV] >= 3) {
        ...
        for (i = EMMC_PART_UNKNOWN; i < EMMC_PART_END; i++) {
...
static void mmc_decode_cid(struct mmc_card *card)
{
    ...
    if (mmc_card_sd(card)) {
        ...
        default:
            dprintf(INFO,"[SD%d] Unknown MMCA version %d\n",
...
static int mmc_decode_csd(struct mmc_card *card)
{
    ...
    if (mmc_card_sd(card)) {
        ...
        case 1:
            ...
            break;
        default:
            dprintf(INFO,"[SD%d] Unknown CSD ver %d\n", mmc_card_id(card), csd_struct);
            return MMC_ERR_INVALID;
        }
    } else {
        /*
...
static void mmc_decode_ext_csd(struct mmc_card *card)
{
    ...
    if ((ext_csd[EXT_CSD_CARD_TYPE] & EXT_CSD_CARD_TYPE_DDR_52_1_2V) ||
        ...
        dprintf(INFO,"[SD%d] MMCv4 but HS unsupported\n", card->host->id);
    }
               ...
               u8 set, u8 index, u8 value)
{
    ...
    do {
        ...
        if (err) {
            dprintf(INFO,"[SD%d] Fail to send status %d\n", host->id, err);
            break;
        }
        if (status & R1_SWITCH_ERROR) {
            dprintf(INFO,"[SD%d] switch error. arg(0x%x)\n", host->id, cmd.arg);
            return MMC_ERR_FAILED;
        }
...
int mmc_switch_hs(struct mmc_host *host, struct mmc_card *card)
{
    ...
    if ((status[16] & 0xF) != 1) {
        dprintf(INFO,"[SD%d] HS mode not supported!\n", host->id);
        err = MMC_ERR_FAILED;
    } else {
...
int mmc_switch_uhs1(struct mmc_host *host, struct mmc_card *card, unsigned int mode)
{
    ...
    if ((status[16] & 0xF) != mode) {
        dprintf(INFO,"[SD%d] UHS-1 %s mode not supported!\n", host->id, smode[mode]);
        err = MMC_ERR_FAILED;
    } else {
        card->uhs_mode = mode;
...
int mmc_switch_drv_type(struct mmc_host *host, struct mmc_card *card, int val)
{
    ...
    if ((status[15] & 0xF) != val) {
        dprintf(INFO,"[SD%d] UHS-1 %s drv not supported!\n", host->id, type[val]);
        err = MMC_ERR_FAILED;
    } else {
...
int mmc_switch_max_cur(struct mmc_host *host, struct mmc_card *card, int val)
{
    ...
    if (((status[15] >> 4) & 0xF) != val) {
        dprintf(INFO,"[SD%d] UHS-1 %s max. current not supported!\n", host->id, curr[val]);
        err = MMC_ERR_FAILED;
    } else {
...
static int mmc_read_scrs(struct mmc_host *host, struct mmc_card *card)
{
    ...
    if (UNSTUFF_BITS(resp, 60, 4) != 0) {
        dprintf(INFO,"[SD%d] Unknown SCR ver %d\n",
...
int mmc_read_ext_csd(struct mmc_host *host, struct mmc_card *card)
{
    ...
    if (card->csd.mmca_vsn < CSD_SPEC_VER_4) {
        dprintf(INFO,"[SD%d] MMCA_VSN: %d. Skip EXT_CSD\n", host->id, card->csd.mmca_vsn);
        return MMC_ERR_NONE;
    }
...
int mmc_sdio_enable_func(struct sdio_func *func)
{
	...
	dprintf(INFO,"SDIO: Enabling device ...\n");
	...
	dprintf(INFO,"SDIO: Enabled device\n");

	return 0;

err:
	dprintf(INFO,"SDIO: Failed to enable device\n");
	return ret;
}
...
int mmc_sdio_disable_func(struct sdio_func *func)
{
	...
	dprintf(INFO,"SDIO: Disabling device...\n");
	...
	dprintf(INFO,"SDIO: Disabled device\n");

	return 0;

err:
	dprintf(INFO,"SDIO: Failed to disable device %s\n");
	return MMC_ERR_FAILED;
}
    ...
    */
    ...
    do {
        ...
        if (i >= ARRAY_SIZE(cis_tpl_list)) {
            ...
            *prev = this;
            prev = &this->next;
            dprintf(INFO,"queuing CIS tuple 0x%02x length %u\n", tpl_code, tpl_link);
        } else {
            const struct cis_tpl *tpl = cis_tpl_list + i;
            if (tpl_link < tpl->min_size) {
            dprintf(INFO,"bad CIS tuple 0x%02x (length = %u, expected >= %u)\n",
...
int mmc_sdio_read_cccr(struct mmc_card *card)
{
    ...
    if (cccr_vsn > SDIO_CCCR_REV_1_20) {
        dprintf(INFO,"unrecognised CCCR structure version %d\n", cccr_vsn);
        return MMC_ERR_INVALID;
    }
...
int mmc_sdio_proc_pending_irqs(struct mmc_card *card)
{
    ...
    if (ret) {
        dprintf(INFO,"[SD%d] error %d reading SDIO_CCCR_INTx\n", id, ret);
        return ret;
    }
    ...
    for (i = 1; i <= 7; i++) {
        if (pending & (1 << i)) {
            ...
            if (!func) {
                dprintf(INFO,"[SD%d] pending IRQ for non-existant function\n", id);
                ret = MMC_ERR_INVALID;
            } else if (func->irq_handler) {
                func->irq_handler(func);
                count++;
            } else {
                dprintf(INFO,"[SD%d] pending IRQ with no handler\n", id);
                ret = MMC_ERR_INVALID;
            }
        }
    }
...
int mmc_sdio_claim_irq(struct sdio_func *func, sdio_irq_handler_t *handler)
{
	...
	dprintf(INFO,"SDIO: Enabling IRQ...\n");

	if (func->irq_handler) {
		dprintf(INFO,"SDIO: IRQ already in use.\n");
		return MMC_ERR_FAILED;
	}
...
int mmc_sdio_release_irq(struct sdio_func *func)
{
	...
	dprintf(INFO,"SDIO: Disabling IRQ...\n");
...
static int __mmc_do_erase(struct mmc_host *host, struct mmc_card *card, u64 start_addr,u64 len)
{
	...
	if(end_addr/card->blklen > card->nblks){
			dprintf(INFO,"[MSDC%d]Erase address out of range! start<0x%llx>,len<0x%llx>,card_nblks<0x%x>\n",host->id,start_addr,len,card->nblks);
			return MMC_ERR_INVALID;
		}
	if ((err = mmc_erase_start(card, start_addr))) {
			dprintf(INFO,"[MSDC%d]Set erase start addrees 0x%llx failed,Err<%d>\n",host->id,start_addr,err);
            goto out;
        }
        if((err = mmc_erase_end(card, end_addr))){
    		dprintf(INFO,"[MSDC%d]Set erase end addrees 0x%llx + 0x%llx failed,Err<%d>\n",host->id,start_addr,len,err);
            goto out;
        }
        if ((err = mmc_erase(card, MMC_ERASE_TRIM))){
            dprintf(INFO,"[MSDC%d]Set erase <0x%llx - 0x%llx> failed,Err<%d>\n",host->id,start_addr,start_addr + len,err);
            goto out;        
        }
        dprintf(INFO,"[MSDC%d]0x%llx - 0x%llx Erased\n", host->id,start_addr,start_addr + len);
out:
	return err;
}
int mmc_do_erase(int dev_num,u64 start_addr,u64 len,u32 part_id)
{
	...
	u32 err;
	if((!card) || (!host) ){
		dprintf(INFO,"[mmc_do_erase] card or host null\n");
		return MMC_ERR_INVALID;
	}
	if(!len ){
			dprintf(INFO,"[MSDC%d] invalid erase size! len<0x%llx>\n",host->id,len);
			return MMC_ERR_INVALID;
	}
	if((start_addr % card->blklen) || (len % card->blklen)){
			dprintf(INFO,"[MSDC%d] non-alignment erase address! start<0x%llx>,len<0x%llx>,card_nblks<0x%x>\n",host->id,start_addr,len,card->nblks);
			return MMC_ERR_INVALID;
	}
    ...
    if (mmc_virt_to_phys(s_blknr, &s_blknr, &s_pid) != MMC_ERR_NONE){
			dprintf("[MSDC%d] s_addr trans error blknr<0x%x>\n",host->id,s_blknr);
			return MMC_ERR_INVALID;
		}
	if (mmc_virt_to_phys(e_blknr, &e_blknr, &e_pid) != MMC_ERR_NONE){
			dprintf("[MSDC%d] e_addr trans error blknr<0x%x>\n",host->id,e_blknr);
			return MMC_ERR_INVALID;
		}
	s_pid_o = s_pid;
	...
	for(s_pid = s_pid_o;s_pid <= e_pid;s_pid++){
		if(erase_part[s_pid].blkcnt == 0 || s_pid == EMMC_PART_RPMB)
			...
			if((err = mmc_switch_part(s_pid))){
					dprintf("[MSDC%d] mmc swtich failed.part<%d> error <%d> \n",host->id,s_pid,err);
					return err;
				}
			
            start_addr  = (u64)(erase_part[s_pid].start_blk * card->blklen);
            len    = (u64)((erase_part[s_pid].blkcnt) * card->blklen);
			dprintf("[MSDC%d] mmc erase part<%d> <0x%llx - 0x%llx>start....\n",host->id,s_pid, start_addr, start_addr + len);
            err =  __mmc_do_erase(host, card, start_addr, len);
            if (err){
				dprintf("[MSDC%d] mmc erase failed. part<%d> error <%d> \n",host->id,s_pid,err);
				return err;
			}
			dprintf("[MSDC%d] mmc erase part<%d> <0x%llx - 0x%llx> done.\n",host->id,s_pid, start_addr, start_addr + len);
		}
#else
	if((err = mmc_switch_part(part_id))){
			dprintf(INFO,"[MSDC%d] mmc swtich failed.part<%d> error <%d> \n",host->id,part_id,err);
			return err;
	}

	if( (err =  __mmc_do_erase(host,card,start_addr,len)) ){
			dprintf(INFO,"[MSDC%d] mmc erase failed.error <%d> \n",host->id,err);
			return err;
	}
#endif
	return err;
...
int mmc_set_bus_width(struct mmc_host *host, struct mmc_card *card, int width)
{
    ...
    if (mmc_card_sd(card)) {
        ...
        if (err != MMC_ERR_NONE) {
            dprintf(INFO,"[SD%d] Switch to bus width(%d) failed\n", host->id, arg);
            goto out;
        }
...
#if 0
static int mmc_part_read(struct mmc_card *card, u8 partno, unsigned long blknr, u32 blkcnt, unsigned long *dst)
{
    int err = MMC_ERR_FAILED;
    u8 val;
    u8 *ext_csd = &card->raw_ext_csd[0];
...
static int mmc_part_write(struct mmc_card *card, u8 partno, unsigned long blknr, u32 blkcnt, unsigned long *src)
{
    int err = MMC_ERR_FAILED;
    u8 val;
    u8 *ext_csd = &card->raw_ext_csd[0];
...
int mmc_dev_bread(struct mmc_card *card, unsigned long blknr, u32 blkcnt, u8 *dst)
{
    ...
    do {
        ...
        if (err == MMC_ERR_BADCRC || err == MMC_ERR_ACMD_RSPCRC || err == MMC_ERR_CMD_RSPCRC){
			...
			dprintf(INFO,"[SD%d] Fail to tuning,%s",host->id,(err == MMC_ERR_CMDTUNEFAIL)?"cmd tune failed!\n":"read tune failed!\n");
			break;
			}
    } while (retry--);
...
int mmc_block_read(int dev_num, unsigned long blknr, u32 blkcnt, unsigned long *dst, u32 part_id)
{
    struct mmc_host *host = mmc_get_host(dev_num);
    struct mmc_card *card = mmc_get_card(dev_num);
    u32 blksz    = host->blklen;
    ...
    if (blknr * (blksz / MMC_BLOCK_SIZE) > card->nblks) {
        dprintf(INFO,"[SD%d] Out of block range: blknr(%ld) > sd_blknr(%d)\n", 
...
int mmc_block_write(int dev_num, unsigned long blknr, u32 blkcnt, unsigned long *src, u32 part_id)
{
    struct mmc_host *host = mmc_get_host(dev_num);
    struct mmc_card *card = mmc_get_card(dev_num);
    u32 blksz    = host->blklen;
	...
	//dprintf("virt addr:%ld\n",dev_num,blknr);
	//mmc_virt_switch(blknr, &blknr);
	//dprintf("phys addr:%ld\n",dev_num,blknr);
    if (!blkcnt)
        return MMC_ERR_NONE;
...
int mmc_init_mem_card(struct mmc_host *host, struct mmc_card *card, u32 ocr)
{
     ...
     */
    if (ocr & 0x7F) {
        dprintf(INFO,"[SD%d] card claims to support voltages "
    ...
    if (err != MMC_ERR_NONE) {
        dprintf(INFO,"[SD%d] Fail in SEND_OP_COND cmd\n", id);
        goto out;
    }
    ...
    if (s18a) {
        ...
        if (err != MMC_ERR_NONE) {
            dprintf(INFO,"[SD%d] Fail in SWITCH_VOLT cmd\n", id);
            goto out;
        }
    }
#endif    
    ...
    if (err != MMC_ERR_NONE) {
        dprintf(INFO,"[SD%d] Fail in SEND_CID cmd\n", id);
        goto out;
    }
    ...
    if (err != MMC_ERR_NONE) {
        dprintf(INFO,"[SD%d] Fail in SEND_RCA cmd\n", id);
        goto out;
    }
    ...
    if (err != MMC_ERR_NONE) {
        dprintf(INFO,"[SD%d] Fail in SEND_CSD cmd\n", id);
        goto out;        
    }
    ...
    if (err != MMC_ERR_NONE) {
        dprintf(INFO,"[SD%d] Fail in decode csd\n", id);
        goto out;        
    }
    ...
    if (err != MMC_ERR_NONE) {
        dprintf(INFO,"[SD%d] Fail in select card cmd\n", id);
        goto out;
    }
    ...
    if (mmc_card_sd(card)) {
        ...
        if (err != MMC_ERR_NONE) {
            dprintf(INFO,"[SD%d] Fail in SEND_SCR cmd\n", id);
            goto out;
        }
        ...
        err = mmc_read_ext_csd(host, card);
        if (err != MMC_ERR_NONE) {
            dprintf(INFO,"[SD%d] Fail in SEND_EXT_CSD cmd\n", id);
            goto out;        
        }
        ...
        if ((card->ext_csd.hs_max_dtr != 0) && (host->caps & MMC_CAP_MMC_HIGHSPEED)) {
            ...
            if (err == MMC_ERR_NONE) {
                dprintf(INFO,"[SD%d] Switch to High-Speed mode!\n", host->id);
                mmc_card_set_highspeed(card);
            }
        }
    ...
    if (!(mmc_card_mmc(card) && mmc_card_ddr(card))) {
        ...
        if (err != MMC_ERR_NONE) {
            dprintf(INFO,"[SD%d] Fail in set blklen cmd\n", id);
            goto out;
        }
    }
...
int mmc_init_sdio_card(struct mmc_host *host, struct mmc_card *card, u32 ocr)
{
     ...
     */
    if (ocr & 0x7F) {
    	dprintf(INFO,"card claims to support voltages "
    	       "below the defined range. These will be ignored.\n");
    	ocr &= ~0x7F;
    }
    ...
    err = mmc_select_card(host, card);
    if (err != MMC_ERR_NONE) {
        dprintf(INFO,"[SD%d] Fail in select card\n", id);
        goto out;        
    }
    ...
    if (err != MMC_ERR_NONE) {
        dprintf(INFO,"[SD%d] Fail in reading the common register\n", id);
        goto out;        
    }
    ...
    if (err != MMC_ERR_NONE) {
        dprintf(INFO,"[SD%d] Fail in reading the common CIS\n", id);
        goto out;        
    }
    ...
    if (err != MMC_ERR_NONE) {
        dprintf(INFO,"[SD%d] Fail in enable SDIO HS\n", id);
        goto out;        
    }
    ...
    dprintf(INFO,"[SD%d] SDIO Initialized\n", host->id);    
...
static int msdc_ett_offline_to_ub(struct mmc_host *host, struct mmc_card *card)
{
    ...
    if (ret) dprintf("msdc failed to find\n");
    return ret;        	
}
#endif
int mmc_init_card(struct mmc_host *host, struct mmc_card *card)
{
    ...
    if (mmc_send_io_op_cond(host, 0, &ocr) == MMC_ERR_NONE) {
        ...
        if (err != MMC_ERR_NONE) {
            dprintf(INFO,"[SD%d] Fail in init sdio card\n", id);
            goto out;        
        }
        /* no memory present */
        if ((ocr & 0x08000000) == 0) {
    ...
    if (err != MMC_ERR_NONE) {        
        ...
        if (err != MMC_ERR_NONE) {
            dprintf(INFO,"[SD%d] Fail in SEND_IF_COND cmd\n", id);
            goto out;
        }
...
#else
unsigned long mmc_wrap_bread(int dev_num, unsigned long blknr, lbaint_t blkcnt, void *dst, u32 part_id)
{
    return mmc_block_read(dev_num, blknr, (u32)blkcnt, (unsigned long*)dst, part_id) == MMC_ERR_NONE ? blkcnt : (unsigned long) -1;
}

unsigned long mmc_wrap_bwrite(int dev_num, unsigned long blknr, lbaint_t blkcnt, const void *src, u32 part_id)
{
    return mmc_block_write(dev_num, blknr, (u32)blkcnt, (unsigned long*)src, part_id) == MMC_ERR_NONE ? blkcnt : (unsigned long) -1;
}
#if 0
...
int mmc_legacy_init(int verbose)
{
    ...
    card = &sd_card[id];
    bdev = &sd_dev[id];
    dprintf(INFO,"[%s]: start mmc_init_host\n", __func__);
    err = mmc_init_host(host, id);

    if (err == MMC_ERR_NONE){
        dprintf(INFO,"[%s]: start mmc_init_card\n", __func__);
        err = mmc_init_card(host, card);
    }
    ...
    if (err == MMC_ERR_NONE && !boot_dev_found) {
    		...
    		mmc_test(0, NULL);
		#endif
		dprintf(INFO,"[%s]:addr_trans_init\n", __func__);
		mmc_addr_trans_tbl_init();
		dprintf(INFO,"[%s]:addr_trans_init done\n", __func__);
        ...
        if (host->boot_type == RAW_BOOT) {
            ...
            boot_dev_found = 1;
            dprintf(INFO,"[SD%d] boot device found\n", id);
        } else if (host->boot_type == FAT_BOOT) {
            #if (CONFIG_COMMANDS & CFG_CMD_FAT)
            if (0 == fat_register_device(bdev, 1)) {
                boot_dev_found = 1;
                dprintf(INFO,"[SD%d] FAT partition found\n", id);
            }
            #endif
        }
    }
