...
bool find_mirror_pt_from_bottom(int *start_addr, part_dev_t *dev)
{
	...
	unsigned char pmt_spare[4];
    unsigned char *head = page_buf;
    unsigned char *tail = pmt_spare;
	...
	for (mpt_locate = (block_size / page_size); mpt_locate > 0; mpt_locate--) {
		...
		if (is_valid_mpt(head) && is_valid_mpt(tail)) {
...
int load_exist_part_tab(u8 *buf, part_dev_t *dev)
{
	...
	int reval=DM_ERR_OK;
	int mirror_address;
	unsigned char pmt_spare[PT_SIG_SIZE];
    unsigned char *head = page_buf;
    unsigned char *tail = pmt_spare;
	...
	for (pt_locate=0; pt_locate < (int)(block_size / page_size); pt_locate++) {
		...
		if (is_valid_pt(head) && is_valid_pt(tail)) {
			pi.sequencenumber = page_buf[PT_SIG_SIZE+page_size];
	...
	if (pt_locate == (int)(block_size/page_size)) {
