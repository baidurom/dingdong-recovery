...
{
    dprintf("\t\tCount      : %d\n", rpt->count);
    dprintf("\t\tMax. Time  : %d counts\n", rpt->max_time);
    dprintf("\t\tMin. Time  : %d counts\n", rpt->min_time);
    dprintf("\t\tTotal Size : %d KB\n", rpt->total_size / 1024);
    dprintf("\t\tTotal Time : %d counts\n", rpt->total_time);
    if (rpt->total_time) {
        dprintf("\t\tPerformance: %d KB/sec\n", 
...
int mmc_prof_dump(int dev_id)
{
    ...
    dprintf("\tSD Host ID     : %d\n", dev_id);
    dprintf("\tOP Clock Freq. : %d khz\n", host->clk / 1000);    
    dprintf("\tSD Clock Freq. : %d khz\n", host->sclk / 1000);
    dprintf("\tCard Type      : %s card\n", (card->type == MMC_TYPE_MMC) ? "MMC" : "SD/SDHC/SDXC");
    dprintf("\tCard Mode      : UHS1(%d) DDR(%d) HS(%d)\n", 
        mmc_card_uhs1(card) ? 1 : 0, mmc_card_ddr(card) ? 1 : 0, 
        mmc_card_highspeed(card) ? 1 : 0);
    dprintf("\tCard Size      : %d MB\n", (card->nblks * card->blklen) / 1024 / 1024);
    dprintf("\tCard Max. Freq.: %d khz\n", card->maxhz / 1000);
    
    if (perf->multi_blks_read.count) {
        dprintf("\tMulti-Blks-Read:\n");
    ...
    if (total_read_time) {
        dprintf("\tPerformance Read : %d KB/sec\n",
            ((total_read_size / 1024) * 32768) / total_read_time);
    }
    if (total_write_time) {
        dprintf("\tPerformance Write: %d KB/sec\n",
...
int mmc_download(int dev_id, u32 imgaddr, u32 size, u32 addr, int bootarea)
{
    ...
    for (i = 0; i < chunks; i++) {
        ...
        dprintf("[SD%d] Write %3d blocks from 0x%.8x(RAM) to 0x%.8x(FLASH).\n",
    ...
    if (!result) {
        dprintf("[SD%d] Download %d blocks (%d bytes) to 0x%.8x successfully\n", 
            dev_id, total_blks, total_blks * MMC_BLOCK_SIZE, blknr * MMC_BLOCK_SIZE);
    } else {
        dprintf("[SD%d] Download %d blocks (%d bytes) to 0x%.8x failed %d\n", 
...
int mmc_download_part(int dev_id, char *part_name, int bootarea)
{
    ...
    if (part) {
        dprintf("[SD%d] Waiting for '%s' image loading from ICE...\n", dev_id, part_name);
...
int mmc_erase_blks(int dev_id, u64 addr, u32 size, int bootarea)
{
    ...
    if (!result) {
        dprintf("[SD%d] Erase %lld blocks (%lld bytes) from 0x%x successfully\n", 
            dev_id, total_blks, total_blks * MMC_BLOCK_SIZE, blknr * MMC_BLOCK_SIZE);
    } else {
        dprintf("[SD%d] Erase %lld blocks (%lld bytes) from 0x%x failed %d\n", 
            dev_id, total_blks, total_blks * MMC_BLOCK_SIZE, blknr * MMC_BLOCK_SIZE, result);
    }    
    return result;
}
...
int mmc_test_mem_card(struct mmc_test_config *cfg)
{
    ...
    while (forever || count--) {
        dprintf("[TST] ==============================================\n");
        dprintf("[TST] BEGIN: %d/%d, No Stop(%d)\n", 
            (cfg->count != -1) ? cfg->count - count : 0, 
            (cfg->count != -1) ? cfg->count : 0, forever);
        dprintf("[TST] ----------------------------------------------\n");
        dprintf("[TST] Mode    : %d\n", cfg->mode);
        dprintf("[TST] Clock   : %d kHz\n", cfg->clock / 1000);
        dprintf("[TST] BusWidth: %d bits\n", cfg->buswidth);
        dprintf("[TST] BurstSz : %d bytes\n", 0x1 << cfg->burstsz);
        dprintf("[TST] BlkAddr : %xh\n", blknr);
        dprintf("[TST] BlkSize : %dbytes\n", blksz);
        dprintf("[TST] TstBlks : %d\n", total_blks);
#if defined(BB_MT6589)
        dprintf("[TST] AutoCMD : 12(%d), 23(%d)\n", 
        ...
        if (mmc_card_sd(card) && cfg->buswidth == HOST_BUS_WIDTH_8) {
            dprintf("[TST] SD card doesn't support 8-bit bus width (SKIP)\n");
            result = MMC_ERR_NONE;
        }
        ...
        if (cfg->tst_single) {
            ...
            for (i = 0; i < total_blks; i++) {
                ...
                if (ret != MMC_ERR_NONE) {
                    dprintf("test single block write failed (%d)\n", i);
                    result = -__LINE__;
                    goto failure;
                }
            }

            dprintf(TC_MSG, host->id, result == 0 ? "PASS" : "FAIL", tid++, 
            ...
            if (result) {
                dprintf("[SD%d]\t\tread back pattern(0x%.2x) failed\n", 
                    id, pattern);
                goto failure;
            }
        }
        ...
        if (cfg->tst_multiple) {
            ...
            for (i = 0; i < chunks; i++) {
                ...
                ret = mmc_block_read(id, blknr + i * chunk_blks, 
                    chunk_blks, (unsigned long*)buf);
                if (ret != MMC_ERR_NONE) {
                    dprintf("[SD%d]\t\tread %d blks failed(ret = %d blks)\n",
...
failure:
        if (result) {
            dprintf("[SD%d] mmc test failed (%d)\n", host->id, result);
            fail++;
        } else {
            pass++;
        }
        dprintf("[TST] ----------------------------------------------\n");
        dprintf("[TST] Test Result: TOTAL(%d/%d), PASS(%d), FAIL(%d) \n", 
...
#ifdef MMC_BOOT_TEST
    dprintf("[EMMC] Boot up with power reset (MMCv4.3 above)\n");
    mmc_boot_up(0, EMMC_BOOT_PWR_RESET);
    mdelay(100);
	for (i = 0; i < 20; i++) {
        dprintf("[EMMC] Boot up with RST_n reset (MMCv4.41 above)\n");
        mmc_boot_up(0, EMMC_BOOT_RST_N_SIG);
        mdelay(100);
    }
    dprintf("[EMMC] Boot up with PRE_IDLE_CMD reset (MMCv4.41 above)\n");
