...
static bool nand_RFIFOValidSize(u16 u2Size)
{
    ...
    if (u2Size == 0)
    {
        while (FIFO_RD_REMAIN(DRV_Reg16(NFI_FIFOSTA_REG16)))
        {
            ...
            if (0 == timeout)
            {
                dprintf(CRITICAL,"nand_RFIFOValidSize failed: 0x%x\n", u2Size);
                return false;
            }
        }
    }
...
static bool nand_dma_read_data(u8 * pDataBuf, u32 u4Size)
{
	...
	if(event_wait_timeout(&nand_int_event,100)){
		dprintf(CRITICAL,"[nand_dma_read_data]wait for AHB done timeout\n");
		dump_nfi();
		return false;
	}
...
static bool nand_dma_write_data(u8 * buf, u32 length)
{
    ...
    {
        //dprintf(CRITICAL,"Un-16-aligned address\n");
    ...
    if(en_interrupt){
	if(event_wait_timeout(&nand_int_event,100)){
		dprintf(CRITICAL,"[nand_dma_write_data]wait for AHB done timeout\n");
		dump_nfi();
                return false;
	}
    }else{
    while (!(DRV_Reg16(NFI_INTR_REG16) & INTR_AHB_DONE))
    {
        ...
        if (0 == timeout)
        {
            dprintf(CRITICAL,"wait write AHB done timeout\n");
            dump_nfi();
	    return FALSE;  
        }
    }
...
static bool nand_mcu_write_data(const u8 * buf, u32 length)
{
    ...
    if ((u32) buf % 4 || length % 4)
    {
        for (i = 0; (i < (length)) && (timeout > 0);)
        {
            ...
            if (0 == timeout)
            {
                dprintf(CRITICAL,"[%s] nand mcu write timeout\n", __FUNCTION__);
                dump_nfi();
                return false;
            }
        }
...
static bool nand_read_oob_raw(struct nand_chip *chip, u32 page_addr, u32 length, u8 * buf)
{
    ...
    {
        dprintf(CRITICAL,"[%s] invalid parameter, length: %d, buf: %p\n", __FUNCTION__, length, buf);
        return false;
    }
...
bool nand_block_bad_hw(struct nand_chip * nand, u32 offset)
{
    ...
    {
        dprintf(CRITICAL,"nand_read_oob_raw return fail\n");
    }
...
static void nand_command_bp(struct nand_chip *nand_chip, unsigned command, int column, int page_addr)
{
    ...
    switch (command)
    {
      ...
      case NAND_CMD_READ_ID:
          ...
          break;
      default:
          dprintf(CRITICAL,"[ERR] nand_command_bp : unknow command %d\n", command);
          break;
    }
}
...
void lk_nand_irq_handler(unsigned int irq)
{
	...
	if(sts & inte){	
	//	dprintf(CRITICAL,"[lk_nand_irq_handler]send event,\n");
...
int nand_init_device(struct nand_chip *nand)
{
	...
	if(spare_per_sec>=28){
		...
		dprintf(CRITICAL,"[NAND]: NFI not support oobsize: %x\n", spare_per_sec);
		while(1);
		return -1;
	}
...
void nand_driver_test(void){
    ...
    dprintf(SPEW,"~~~~~~~~~nand driver test in lk~~~~~~~~~~~~~~\n");
    len = dev->read(dev, start_addr, (uchar *) original, test_len);
    if (len != test_len)
    {
        dprintf(SPEW,"read original fail %d\n", len);
    }
    dprintf(SPEW,"oringinal data:");
    for (index = 0; index < 300; index++)
    {
        dprintf(SPEW," %x", original[index]);
    }
    dprintf(SPEW,"\n");
    len = dev->write(dev, (uchar *) source, start_addr, test_len);
    if (len != test_len)
    {
        dprintf(SPEW,"write source fail %d\n", len);
    }
    len = dev->read(dev, start_addr, (uchar *) readback,test_len);
    if (len != test_len)
    {
        dprintf(SPEW,"read back fail %d\n", len);
    }
    dprintf(SPEW,"readback data:");
    for (index = 0; index < 300; index++)
    {
        dprintf(SPEW," %x", readback[index]);
    }
    dprintf(SPEW,"\n");
    for (index = 0; index < test_len; index++)
    {
        ...
        {
            dprintf(SPEW,"compare fail %d\n", index);
		fail=1;
            break;
        }        
    }
    if(fail==0){
	dprintf(SPEW,"compare success!\n");
    }
    len = dev->write(dev, (uchar *) original, start_addr, test_len);
    if (len != test_len)
    {
        dprintf(SPEW,"write back fail %d\n", len);
    } else
    {
        dprintf(SPEW,"recovery success\n");
    }
    ...
    dprintf(SPEW,"read back oringinal data:");
    for (index = 0; index < 300; index++)
    {
        dprintf(SPEW," %x", original[index]);
    }
...
int nand_erase(u64 offset, u64 size)
{
    ...
    dprintf (SPEW,"[ERASE] image size = 0x%x\n", img_size);
    dprintf (SPEW,"[ERASE] the number of nand block of this image = %d\n", tblksz);
    ...
    while (tblksz != 0)
    {
        ...
        {
            dprintf(CRITICAL,"[ERASE] erase 0x%x fail\n",cur_offset);
        ...
        {
            dprintf(CRITICAL,"[ERASE] cur offset (0x%x) exceeds erase limit address (0x%x)\n", cur_offset, total_size);
            return 0;
        }
    }
...
bool __nand_erase (u32 logical_addr)
{
    ...
    {
        dprintf(CRITICAL,"erase block 0x%x failed\n", mapped_block);
        if(update_bmt(mapped_block * g_nand_chip.erasesize, UPDATE_ERASE_FAIL, NULL, NULL)){
			...
			dprintf(CRITICAL,"erase block 0x%x failed but update bmt fail\n",mapped_block);
			return FALSE;
		}
    }
...
int nand_write_img(u32 addr, void *data, u32 img_sz,u32 partition_size,int img_type)
{
	...
	dprintf(SPEW,"[nand_wite_img]write to addr %x,img size %x\n",addr,img_sz);
	if(addr % block_size || partition_size % block_size){
		dprintf(CRITICAL,"[nand_write_img]partition address or partition size is not block size alignment\n");
		return -1;
	}
	if(img_sz > partition_size){
		dprintf(CRITICAL,"[nand_write_img]img size %x exceed partition size\n",img_sz);
		return -1;
	}
	if(page_size == 4096){
		img_spare_size = 128;
	...
	if(img_type == YFFS2_IMG){
		...
		if(img_sz % write_size){
			dprintf(CRITICAL,"[nand_write_img]img size is not w_size %d alignment\n",write_size);
			return -1;
		}	
	}else{
	...
	while(img_sz>0){
		...
		if((addr+img_sz)>partition_end){
			dprintf(CRITICAL,"[nand_wite_img]write to addr %x,img size %x exceed parition size,may be so many bad blocks\n",addr,img_sz);
			return -1;
		}
		...
		if((img_sz < write_size)){
			...
			if(img_type == YFFS2_IMG){
				...
				if((img_type == UBIFS_IMG)&& (check_data_empty((void *)g_data_buf,page_size))){
					dprintf(SPEW,"[nand_write_img]skip empty page\n");
					ret = true;
				}else{
		...
		if(ret == FALSE){
			dprintf(CRITICAL,"[nand_write_img]write fail at % 0x%x\n",addr);
			if (__nand_erase(addr) == FALSE)
     		{
	            dprintf(CRITICAL,"[ERASE] erase 0x%x fail\n",addr);
	            mark_block_bad (addr);
...
int nand_write_img_ex(u32 addr, void *data, u32 length,u32 total_size, u32 *next_offset, u32 partition_start,u32 partition_size, int img_type)
{
	...
	u32 last_addr = addr;
	u32 dst_block = 0;
	dprintf(SPEW,"[nand_write_img_ex]write to addr %x,img size %x, img_type %d\n",addr,length,img_type);
	if(partition_start % block_size || partition_size % block_size){
		dprintf(SPEW,"[nand_write_img_ex]partition address or partition size is not block size alignment\n");
		return -1;
	}
	if(length > partition_size){
		dprintf(SPEW,"[nand_write_img_ex]img size %x exceed partition size\n",length);
		return -1;
	}
    ...
    if(last_addr % page_size){
		dprintf(SPEW,"[nand_write_img_ex]write addr is not page_size %d alignment\n",page_size);
		return -1;
	}
	if(img_type == YFFS2_IMG){
		write_size = page_size + img_spare_size;
		if(total_size % write_size){
			dprintf(SPEW,"[nand_write_img_ex]total image size %d is not w_size %d alignment\n",total_size,write_size);
			return -1;
		}
	}else{
		write_size = page_size;
	...
	if(addr == partition_start){
		dprintf(SPEW,"[nand_write_img_ex]first chunk\n");
		first_chunk = 1;
		download_size = 0;
		memset(g_data_buf,0xff,write_size);
	}
	if((length + download_size) >= total_size){
		dprintf(SPEW,"[nand_write_img_ex]last chunk\n");
		last_chunk = 1;
	}
	...
	while(length>0){
		...
		if((addr+length)>partition_end){
			dprintf(CRITICAL,"[nand_write_img_ex]write to addr %x,img size %x exceed parition size,may be so many bad blocks\n",addr,length);
			return -1;
		}
		...
		if((addr % block_size)==0){
			if (__nand_erase(addr) == FALSE)
     		{
	            dprintf(SPEW,"[ERASE] erase 0x%x fail\n",addr);
	            mark_block_bad (addr);
           		addr += block_size;
				continue;  //erase fail, skip this block
       	 	}
		...
		if(img_type == YFFS2_IMG){
			...
			if((img_type == UBIFS_IMG)&& (check_data_empty((void *)g_data_buf,page_size))){
				dprintf(SPEW,"[nand_write_img_ex]skip empty page\n");
				ret = true;
			}
			else{
					ret = nand_write_page_hwecc(addr,g_data_buf,NULL);
		...
		/*need to check?*/
		if(ret == FALSE){
			dprintf(CRITICAL,"[nand_write_img_ex]write fail at % 0x%x\n",addr);
			while(1){
				dst_block = find_next_good_block(addr/block_size);
				if(dst_block == 0)
				{
					dprintf(CRITICAL,"[nand_write_img_ex]find next good block fail\n");
					return -1;
				}
				ret = block_replace(addr/block_size,dst_block,addr/page_size);
				if(ret == FALSE){
					dprintf(SPEW,"[nand_write_img_ex]block replace fail,continue\n");
					continue;
				}else{
					dprintf(SPEW,"[nand_write_img_ex]block replace sucess %x--> %x\n",addr/block_size,dst_block);
					break;
				}
...
static bool block_replace(u32 src_block, u32 dst_block, u32 error_page)
{
	...
	if(ret == FALSE){
		dprintf(CRITICAL,"[block_replace]%x-->%x erase fail\n",src_block,dst_block);
	...
	if(!data_buf || !spare_buf){
		dprintf(CRITICAL,"[block_replace]malloc mem fail\n");
		return -1;
	}
