...
static struct msdc_smt msdc_smt_spec[3][3] = {
	{
	{GPIO_SMT_GRP25,	1,	13},
	{GPIO_SMT_GRP26,	0,	13},
	{GPIO_SMT_GRP27,	2,	13}},
			...
			for(j = 0; j < sizeof(msdc_pupd_spec[0])/sizeof(msdc_pupd_spec[0][0]); j++){
				if (pin == msdc_pupd_spec[i][j].pin){
					if (enable == GPIO_PULL_DISABLE){
						if (i == 0) {
							GPIO_SET_BITS((3L << (msdc_pupd_spec[i][j].bit - 2)), &obj->reg->msdc1_ctrl[msdc_pupd_spec[i][j].reg].rst);
						} else if (i == 1) {
							GPIO_SET_BITS((3L << (msdc_pupd_spec[i][j].bit - 2)), &obj->reg->msdc2_ctrl[msdc_pupd_spec[i][j].reg].rst);
						} else if (i == 2) {
							GPIO_SET_BITS((3L << (msdc_pupd_spec[i][j].bit - 2)), &obj->reg->msdc3_ctrl[msdc_pupd_spec[i][j].reg].rst);
						}
					}else{
						if (i == 0) {
							GPIO_SET_BITS((3L << (msdc_pupd_spec[i][j].bit - 2)), &obj->reg->msdc1_ctrl[msdc_pupd_spec[i][j].reg].set);
						} else if (i == 1) {
							GPIO_SET_BITS((3L << (msdc_pupd_spec[i][j].bit - 2)), &obj->reg->msdc2_ctrl[msdc_pupd_spec[i][j].reg].set);
						} else if (i == 2) {
							GPIO_SET_BITS((3L << (msdc_pupd_spec[i][j].bit - 2)), &obj->reg->msdc3_ctrl[msdc_pupd_spec[i][j].reg].set);
						}
					}
					return RSUCCESS;
				}
			...
			for(j = 0; j < sizeof(msdc_pupd_spec[0])/sizeof(msdc_pupd_spec[0][0]); j++){
				if (pin == msdc_pupd_spec[i][j].pin){
					if (i == 0) {
						...
						reg = GPIO_RD32(&obj->reg->msdc3_ctrl[msdc_pupd_spec[i][j].reg].val);
					}
				}
				return (((reg & (3L << (msdc_pupd_spec[i][j].bit - 2))) == 0)? 0: 1);
			}
		}
	}else if((pin >= GPIO141)&&(pin <= GPIO166)){
		return GPIO_PULL_EN_UNSUPPORTED;
				...
				if (pin == msdc_pupd_spec[i][j].pin){
					if (select == GPIO_PULL_DOWN){
						if (i == 0) {
							...
							GPIO_SET_BITS((1L << msdc_pupd_spec[i][j].bit), &obj->reg->msdc3_ctrl[msdc_pupd_spec[i][j].reg].set);
						}
					}else{
						if (i == 0) {
							...
							GPIO_SET_BITS((1L << msdc_pupd_spec[i][j].bit), &obj->reg->msdc3_ctrl[msdc_pupd_spec[i][j].reg].rst);
						}
					}
					return RSUCCESS;
				}
...
/*---------------------------------------------------------------------------*/
s32 mt_set_gpio_smt_chip(u32 grp, u32 enable)
{
    u32 pos;
    u32 bit;
	u32 i, j;
    ...
    if (grp >= GPIO_SMT_GRP_MAX)
        return -ERINVAL;

    if (enable >= GPIO_SMT_MAX)
        return -ERINVAL;

	if (grp <= GPIO_SMT_GRP24){
		pos = grp / MAX_GPIO_REG_BITS;
		bit = grp % MAX_GPIO_REG_BITS;

		if (enable == GPIO_SMT_ENABLE)
			GPIO_SET_BITS((1L << bit), &obj->reg->smt[pos].set);
		else
			GPIO_SET_BITS((1L << bit), &obj->reg->smt[pos].rst);
	}else{
		for(i = 0; i < sizeof(msdc_smt_spec)/sizeof(msdc_smt_spec[0]); i++){
			for(j = 0; j < sizeof(msdc_smt_spec[0])/sizeof(msdc_smt_spec[0][0]); j++){
				if (grp == msdc_smt_spec[i][j].grp){
					if (enable == GPIO_SMT_DISABLE){
						if (i == 0) {
							GPIO_SET_BITS((1L << msdc_smt_spec[i][j].bit), &obj->reg->msdc2_ctrl[msdc_smt_spec[i][j].reg].rst);
						} else if (i == 1) {
							GPIO_SET_BITS((1L << msdc_smt_spec[i][j].bit), &obj->reg->msdc1_ctrl[msdc_smt_spec[i][j].reg].rst);
						} else if (i == 2) {
							GPIO_SET_BITS((1L << msdc_smt_spec[i][j].bit), &obj->reg->msdc0_ctrl[msdc_smt_spec[i][j].reg].rst);
						}
					}else{
						if (i == 0) {
							GPIO_SET_BITS((1L << msdc_smt_spec[i][j].bit), &obj->reg->msdc2_ctrl[msdc_smt_spec[i][j].reg].set);
						} else if (i == 1) {
							GPIO_SET_BITS((1L << msdc_smt_spec[i][j].bit), &obj->reg->msdc1_ctrl[msdc_smt_spec[i][j].reg].set);
						} else if (i == 2) {
							GPIO_SET_BITS((1L << msdc_smt_spec[i][j].bit), &obj->reg->msdc0_ctrl[msdc_smt_spec[i][j].reg].set);
						}
					}
				}
			}
...
/*---------------------------------------------------------------------------*/
s32 mt_set_gpio_smt(u32 grp, u32 enable)
{
    return mt_set_gpio_smt_chip(grp,enable);
}
/*****************************************************************************/
/* sysfs operation                                                           */
/*****************************************************************************/
