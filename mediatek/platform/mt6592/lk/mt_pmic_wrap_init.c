...
static S32 _pwrap_init_dio( U32 dio_en )
{
  ...
  pwrap_write_nochk(WRAP_DEW_DIO_EN, dio_en);
  ...
  // Read Test
  pwrap_read_nochk(WRAP_DEW_READ_TEST,&rdata);
...
static S32 _pwrap_init_cipher( void )
{
  ...
  //Config CIPHER @ PMIC
  pwrap_write_nochk(WRAP_DEW_CIPHER_SWRST,   0x1);
  pwrap_write_nochk(WRAP_DEW_CIPHER_SWRST,   0x0);
  pwrap_write_nochk(WRAP_DEW_CIPHER_KEY_SEL, 0x1);
  pwrap_write_nochk(WRAP_DEW_CIPHER_IV_SEL,  0x2);
#ifdef SLV_6320
  pwrap_write_nochk(WRAP_DEW_CIPHER_LOAD,    0x1);
  pwrap_write_nochk(WRAP_DEW_CIPHER_START,   0x1);
#elif defined SLV_6323
  pwrap_write_nochk(WRAP_DEW_CIPHER_EN,  0x1);
#endif
  ...
  do
  {
    pwrap_read_nochk(WRAP_DEW_CIPHER_RDY,&rdata);
    if (_pwrap_timeout_ns(start_time_ns, timeout_ns))
    {
  ...
  pwrap_write_nochk(WRAP_DEW_CIPHER_MODE, 0x1);
...
static S32 _pwrap_init_sistrobe( void )
{
  ...
  for( ind=0; ind<24; ind++)  // 24 sampling clock edge
  {
    ...
    _pwrap_wacs2_nochk(0, WRAP_DEW_READ_TEST, 0, &rdata);
    if( rdata == DEFAULT_VALUE_READ_TEST )
    {
  ...
  if( (tmp1 - tmp2) != result )
  //if((U32)(((tmp1 - tmp2)>=0)?(tmp1 - tmp2):(tmp2 - tmp1)) != result)
  {
...
static S32 _pwrap_init_reg_clock( U32 regck_sel )
{
  ...
  if( regck_sel == 1 )
    ...
    pwrap_read_nochk(PMIC_TOP_CKCON2, &rdata);
  #elif defined SLV_6323
    pwrap_write_nochk(WRAP_TOP_CKCON1_CLR, wdata);
    pwrap_read_nochk(WRAP_TOP_CKCON1,  &rdata);
  #endif
  ...
  #elif defined SLV_6323
    pwrap_write_nochk(WRAP_DEW_RDDMY_NO, 0x8);
...
S32 pwrap_init ( void )
{
  ...
  //###############################
  sub_return = pwrap_write_nochk(WRAP_DEW_WRITE_TEST, WRITE_TEST_VALUE);
  sub_return1 = pwrap_read_nochk(WRAP_DEW_WRITE_TEST,&rdata);
  ...
  //###############################
  //_pwrap_wacs2_nochk(1, WRAP_DEW_WRITE_TEST, 0x5678, &rdata);
  //WRAP_WR32(PMIC_WRAP_SIG_ADR,WRAP_DEW_WRITE_TEST);
  ...
  // should be the last to modify WRITE_TEST
  //###############################
  sub_return=pwrap_write_nochk(WRAP_DEW_CRC_EN, 0x1);
  if( sub_return != 0 )
  {
  ...
  WRAP_WR32(PMIC_WRAP_SIG_ADR , WRAP_DEW_CRC_VAL);
  ...
  // PMIC_DEWRAP enables
  //###############################
  //sub_return  = pwrap_write_nochk(WRAP_DEW_EVENT_OUT_EN, 0x1);
  //sub_return1 = pwrap_write_nochk(WRAP_DEW_EVENT_SRC_EN, 0xffff);
#if 0
 if(( sub_return != 0 )||( sub_return1 != 0 ))
  {
  ...
  //###############################
#ifdef SLV_6323
  WRAP_WR32(PMIC_WRAP_ADC_CMD_ADDR    , WRAP_AUXADC_CON21);
  WRAP_WR32(PMIC_WRAP_PWRAP_ADC_CMD   , 0x8000);
  WRAP_WR32(PMIC_WRAP_ADC_RDY_ADDR    , WRAP_AUXADC_ADC12);
  WRAP_WR32(PMIC_WRAP_ADC_RDATA_ADDR1 , WRAP_AUXADC_ADC13);
  WRAP_WR32(PMIC_WRAP_ADC_RDATA_ADDR2 , WRAP_AUXADC_ADC14);
#endif
...
static S32 _pwrap_status_update_test_porting( void )
{
  ...
  pwrap_write(WRAP_DEW_WRITE_TEST, WRITE_TEST_VALUE);
  WRAP_WR32(PMIC_WRAP_SIG_ADR,WRAP_DEW_WRITE_TEST);
