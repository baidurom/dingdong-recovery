...
void vH264DecEndProc(UINT32 u4InstID)
{
 ...
 /*       if(u4InstID == 0)
  ...
  do_gettimeofday(&t1);
  _u4DecTime[1] = t1.tv_sec * 1000000ULL + t1.tv_usec;
  printk("Decoding time = %llu - %llu = %llu us\n", _u4DecTime[1], _u4DecTime[0], _u4DecTime[1] - _u4DecTime[0]);

#if (POWER_TEST_CASE == POWER_TEST_DCM)
    printk("VDEC CLK = 0x%x, SMI CLK = 0x%x\n", u4ReadGconReg(0), u4ReadGconReg(8));
