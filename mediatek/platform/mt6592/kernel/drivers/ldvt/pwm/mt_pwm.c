...
int mt6592_pwm_open(struct inode *node, struct file *file)
{
	if (!pwm_dev) {
...
int mt6592_pwm_release(struct inode *node, struct file *file)
{
	if (!pwm_dev) {
		PWMDBG("device is invalid.\n");
		return -EBADADDR;
...
struct file_operations mt6592_pwm_fops={
	.owner = THIS_MODULE,
	.open=mt6592_pwm_open,
	.release=mt6592_pwm_release,
};

static struct miscdevice mt6592_pwm_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "mt6592_pwm",
	.fops = &mt6592_pwm_fops,
};
...
MODULE_DESCRIPTION(" This module is for mt6592 chip of mediatek");
