...
INNO_RET INNO_SPI_Write_Byte_Type2(unsigned long addr, unsigned char *data, int len)  //cmd = 0x73
{
	...
	if( len > MT6589_CMMB_SPI_TX_MAX_PKT_LENGTH_PER_TIMES )
	   	{
			pkt_count = len / MT6589_CMMB_SPI_TX_MAX_PKT_LENGTH_PER_TIMES;
			remainder = len % MT6589_CMMB_SPI_TX_MAX_PKT_LENGTH_PER_TIMES;
			ptransfer = kmalloc((pkt_count+1)*sizeof(struct spi_transfer), GFP_KERNEL);
...
#else
	pkt_count = len / CMMB_SPI_INTERFACE_MAX_PKT_LENGTH_PER_TIMES;
	remainder = len % CMMB_SPI_INTERFACE_MAX_PKT_LENGTH_PER_TIMES;
	if(len>CMMB_SPI_INTERFACE_MAX_PKT_LENGTH_PER_TIMES){	
