...
int mali_sync_timed_commit(struct sync_pt *pt)
{
	...
	{
        MALI_PRINTF(("mali_sync_timeline is not ours!\n"));
        if (pt->parent)
        {
            MALI_PRINTF(("pt = %p, parent name = %s, ", pt, pt->parent->name));
            if (pt->parent->ops)
            {
                MALI_PRINTF(("parent driver name = %s, ", pt->parent->ops->driver_name));
            }
            else
            {
                MALI_PRINTF(("parent driver name was NULL, "));
            }
            if (pt->fence)
            {
                MALI_PRINTF(("fence name = %s\n", pt->fence->name));
            }
            else
            {
                MALI_PRINTF(("fence name was NULL\n"));
            }
        }
        else
        {
            MALI_PRINTF(("pt = %p, parent was NULL\n", pt));
        }

#ifdef CONFIG_MTK_AEE_FEATURE
        aee_kernel_warning("3D mali_sync_timeline is not ours!", "mali_sync_timeline is not ours!");
#endif
		return -EINVAL;
	}
	...
	if (0 == ret)
	{
        MALI_PRINTF(("del_timer_sync return 0!\n"));
#ifndef ANR_ERROR_HANDLING
        return -ETIME;
#else
		return -ETIMEDOUT;
