...
#define OSK_LOCK_MAX 32
struct lock_class_key osk_mutex_key[OSK_LOCK_MAX];
static int osk_lock_num = 0;
struct lock_class_key osk_spinlock_key[OSK_LOCK_MAX];
static int osk_spinlock_num = 0;
...
_mali_osk_lock_t *_mali_osk_lock_init( _mali_osk_lock_flags_t flags, u32 initial, u32 order )
{
	...
	if ( (flags & _MALI_OSK_LOCKFLAG_SPINLOCK) )
	{
        ...
        /* The solution for the false alarm of dynamic init locks */
        if (osk_spinlock_num < OSK_LOCK_MAX)
        {
            lockdep_set_class(&lock->obj.spinlock, &osk_spinlock_key[osk_spinlock_num++]);
        }
        else
        {
            MALI_PRINTF(("Not enough spin lock key\n"));
        }
	}
	...
	else
	{
        ...
        /* The solution for the false alarm of dynamic init locks */
        if (osk_lock_num < OSK_LOCK_MAX)
        {
            lockdep_set_class(&lock->obj.mutex, &osk_mutex_key[osk_lock_num]);
            ++osk_lock_num;
        }
        else
        {
            MALI_PRINTF(("Not enough lock key\n"));
        }
	}
