	...
	if (pmic_type == MT65XX_LED_PMIC_LCD_ISINK)
	{
		...
		else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK3)
		{
			if((button_flag_isink3==0) && (first_time == true)) {//button flag ==0, means this ISINK is not for button backlight
				...
				upmu_set_rg_isink3_ck_sel(0);
				upmu_set_isink_ch3_mode(PMIC_PWM_0);
	//			upmu_set_isink_ch3_step(0x3);//16mA //modify by dongzhonglin 20131225 for S610T
				upmu_set_isink_ch3_step(0x5);//24mA
				//hwPWMsetting(PMIC_PWM_1, 15, 8);
				//upmu_set_isink_dim3_duty(0); //chenqi@wind-mobi.com comment 20140218
				upmu_set_isink_dim3_fsel(0);//6323 1KHz


			//chenqi@wind-mobi.com 20140218 modify begin
			//button backlight dim on/off
			if (level) 
			{
				int i;
				upmu_set_isink_dim3_duty(0);
				upmu_set_isink_ch3_en(0x01);
				for(i = 0; i <= 31; i++){
					upmu_set_isink_dim3_duty(i);
					msleep(7);  // 0.5 second dim on
				}								
			}
			else 
			{
				...
				for(i = 31; i >= 0; i--){
					upmu_set_isink_dim3_duty(i);
					msleep(15);  //1 second dim off
				}	
				upmu_set_isink_ch3_en(0x00);
			}
			//chenqi@wind-mobi.com 20140218 modify end
			mutex_unlock(&leds_pmic_mutex);
			return 0;
		}
	  #if 0
