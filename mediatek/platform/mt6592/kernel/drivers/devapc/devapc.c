...
{  
    ...
    */
	...
	if(domain_ID==0){       
		...
		if(r_w_violation == 1)
		{
		  xlog_printk(ANDROID_LOG_ERROR, DEVAPC_TAG ,"Vio Addr:0x%x , Master ID:0x%x , Dom ID:0x%x,W\n", dbg1, master_ID, domain_ID);
		}
		else
		{
		  xlog_printk(ANDROID_LOG_ERROR, DEVAPC_TAG ,"Vio Addr:0x%x, Master ID:0x%x, Dom ID:0x%x,R\n", dbg1, master_ID, domain_ID);
		}
    ...
    if ((dbg0 !=0) || (dbg1 !=0)) 
    {
        xlog_printk(ANDROID_LOG_DEBUG, DEVAPC_TAG ,"[DEVAPC] Multi Violation!\n");
    ...
    xlog_printk(ANDROID_LOG_ERROR, DEVAPC_TAG ,"[DEVAPC]time: %d us!\n",diff);
    */
