...
#endif
	// register cat or echo  /proc/ddp_slts
	struct proc_dir_entry *entry;
    entry = create_proc_entry("slt_result", 0, NULL);
    if (entry) {
        entry->read_proc = Ddp_Read_State;
        entry->write_proc = Ddp_Write_State;
    }

	// register cat /proc/ddp_slts
	create_proc_read_entry("is_suspend",
						   0,
						   NULL,
						   Ddp_Suspend_State,
						   NULL);
