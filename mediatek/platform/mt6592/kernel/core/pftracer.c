...
static int __init pftracer_init(void)
{
	...
	if (err) {
        ...
        if (NR_CPUS>=4)
        {
            err = platform_device_register(&(etm_device[4]));
            if (err) {
                pr_err("Fail to register etm_device 4");
                return err;
            }
            err = platform_device_register(&(etm_device[5]));
            if (err) {
                pr_err("Fail to register etm_device 5");
                return err;
            }
            err = platform_device_register(&(etm_device[6]));
            if (err) {
                pr_err("Fail to register etm_device 6");
                return err;
            }
            err = platform_device_register(&(etm_device[7]));
            if (err) {
                pr_err("Fail to register etm_device 7");
                return err;
            }
        }
#endif        
