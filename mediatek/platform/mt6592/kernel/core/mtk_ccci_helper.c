...
static void ccci_low_battery_cb(LOW_BATTERY_LEVEL level)
{
	int md_id;
	int ret;
	...
	{
	  for(md_id=0;md_id<MAX_MD_NUM;md_id++)
		{
			ret = notify_md_by_sys_msg(md_id, MD_LOW_BATTERY_LEVEL, reserve);
			printk("[ccci/ctl] ccci_low_battery_cb: md_id=%d, MSG=%x, reserve=%d, ret=%d\n",md_id,MD_LOW_BATTERY_LEVEL, reserve, ret);
		}
	}
}
...
static int __init ccci_helper_init(void)
{
	...
	register_low_battery_notify(&ccci_low_battery_cb, LOW_BATTERY_PRIO_MD);
