...
int CamPath_B::setDMACurrBuf( MUINT32 const dmaChannel)
{
    ...
    //
    if ( ISP_DMA_P2_IMG2O & dmaChannel ) {
        this->DMAP2Img2o.dma_cfg.memBuf.base_pAddr = this->ispBufCtrl.getCurrHwBuf((MUINT32) ISP_DMA_P2_IMG2O);
        ret = this->DMAP2Img2o.config();
        if ( 0 != ret ) {
            ISP_PATH_ERR("ERROR config img2o ");
            goto EXIT;
        }
