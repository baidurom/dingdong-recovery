...
typedef struct mtk_layer_data {
    ...
    // Stereo3D related
    uint8_t           s3dLayerFlags;
    uint8_t           s3dBufferFlags;
    int8_t            s3dOffset;
} mtk_layer_data_t;

typedef struct mtk_list_data {
    uint32_t          mflags;

    // Stereo3D related
    uint8_t           s3dOrientation;
    uint8_t           s3dPhase;

    // External Display related
    uint8_t           extOrientation;
} mtk_list_data_t;
