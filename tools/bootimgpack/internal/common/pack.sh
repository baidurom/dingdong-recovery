...
function pack_bootimg()
{
    ...
    BOOTRAMDISKOFFSET=$(cat ./ramdiskaddr)
    
if [[ "$BOOTRAMDISKOFFSET" == "" ]];then
    $MKBOOTIMG --kernel ./kernel --cmdline "$BOOTCMDLINE" --pagesize "$BOOTPAGESIZE"  --base "$BOOTBASE" --ramdisk ./ramdisk.img --output out.img
else
    $MKBOOTIMG --kernel ./kernel --cmdline "$BOOTCMDLINE" --pagesize "$BOOTPAGESIZE"  --base "$BOOTBASE" --ramdiskaddr "$BOOTRAMDISKOFFSET" --ramdisk ./ramdisk.img --output out.img
fi
