    ...
    while (count && mInputReader.readEvent(&event)) {
        ...
        if (type == EV_ABS) {
            if (event->code == EVENT_TYPE_LIGHT) {
                mPendingEvent.light = event->value;//indexToValue(event->value);
...
float LightSensor::indexToValue(size_t index) const
{
    ...
    static const struct adcToLux {
        ...
        { 100, 2600.0 },  /* from 1401 - 1600 adc, we map to  2600.0 lux */
        { 255, 10240.0 }, /* from 1601 - 4095 adc, we map to 10240.0 lux */
    };
    size_t i;
