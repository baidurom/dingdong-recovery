...
int set_bootloader_message_emmc(const struct bootloader_message *in, Volume *v) {
    printf("common updater: set emmc bootloader message:\n");
    printf("command=%s\n",in->command);
    printf("status=%s\n",in->status);
    printf("recovery=%s\n",in->recovery);
...
int set_bootloader_message(const struct bootloader_message *in) {
    size_t write_size;
    printf("common updater: set bootloader message:\n");
    printf("command=%s\n",in->command);
    printf("status=%s\n",in->status);
    printf("recovery=%s\n",in->recovery);
    const MtdPartition *part = mtd_find_partition_by_name(MISC_NAME);
...
int write_update_for_bootloader(
    ...
    printf("common updater: write_update_for_bootloader\n");
...
int set_fota_cookie_mtd(void)
{
	...
	if(write_size < 8)
	{
		 LOGE("the FOTA partition is too small\n");
		 return -1;
	}
