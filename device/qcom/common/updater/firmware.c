...
int install_firmware_update(const char *update_type,
    ...
    printf("common updater: install_firmware_update\n");
    if (update_data == NULL || update_length == 0)
    {
        printf("NULL update: data=%s len=%s\n", update_data,update_length);
        return 0;
    }
...
{
    int result;
    printf("common updater: start_firmware_update\n");
    struct bootloader_message boot;
    ...
    {
		LOGI("Created cookie file for MTD FOTA update\n");
		set_fota_cookie_mtd();

		// Reset to mormal system boot so recovery won't cycle indefinitely.
		if (set_bootloader_message(&boot)) {
			printf("clear bootloader message fail\n");
			return -1;
		}
		LOGI("clear the bootloader message and set MTD FOTA successful\n");
	}
