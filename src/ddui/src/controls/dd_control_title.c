...
ACONTROLP actitle_text(
  ...
  byte isbig
	  ...
	  *ph = txth + pad*2;
	  ...
	  //normal
	  ag_rect(&d->control, 0, 0, w, h, acfg()->winbarbg);
	  ag_rect(&d->control_pushed, 0, 0, w, h, acfg()->winbarbg);
	  ag_rect(&d->control_focused, 0, 0, w, h, acfg()->winbarbg);
	  //normal
	  //
	  ...
	  d->h = txth;
	  d->w = txtw;
	  //ag_textf(&d->control, txtw, txtx+1, txty+1, text, acfg()->wintitlebg, isbig);
	  ag_text(&d->control, txtw, txtx, txty, text, acfg()->wintitlefg, isbig);
	  //focused
	  //ag_textf(&d->control_focused, txtw, txtx+1, txty+1, text, acfg()->wintitlebg, isbig);
	  ag_text(&d->control_focused, txtw, txtx, txty, text, acfg()->wintitlefg, isbig);
	  //pushed
	  //ag_textf(&d->control_pushed, txtw, txtx+1, txty+1, text, acfg()->wintitlebg, isbig);
	  ag_text(&d->control_pushed, txtw, txtx, txty, text, acfg()->wintitlefg, isbig);
...
ACONTROLP actitle(
  ...
  *ph = (txth > imgH)?txth:imgH;
  ...
  if(enableicon)
	  apng_draw_ex(&d->control, image, imgx, 0, 0, 0, imgW, imgH);
  //focused
  memset(image, 0, sizeof(PNGCANVAS));
  if (!apng_load(image, image_back_focused)){
      dd_debug("load %s failed\n", image_back_focused);
      goto apng_load_fail;
  } 
  imgW = image->w;
  imgH = image->h;
  imgx = winW - imgW - txtx*2;
  if(enableicon)
  	  apng_draw_ex(&d->control_focused, image,  imgx, 0, 0, 0, imgW, imgH);
  //pushed 
  memset(image, 0, sizeof(PNGCANVAS));
  ...
  imgx = winW - imgW - txtx*2;
  if(enableicon)
  	  apng_draw_ex(&d->control_pushed, image, imgx, 0, 0, 0, imgW, imgH);
apng_load_fail:
  free(image);
  image = NULL;
  //normal
