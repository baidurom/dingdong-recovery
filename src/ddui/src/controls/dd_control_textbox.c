...
void actext_rebuild(
  ...
  int ch            = 0;
  if (text!=NULL)
    ch = ag_txtheight(cw-agdp()*minpadding*5,text,isbig)+(agdp()*(minpadding*2));
  else
    ch = h;
  ...
  //-- Draw Client
  ag_rect(&d->client,0,0,cw,ch,acfg()->titlefg);
  ag_rect(&d->control, 0, 0, w, h, acfg()->titlefg);
  ag_rect(&d->control_focused, 0, 0, w, h, acfg()->titlefg);

  if (text!=NULL)
	  ag_text(&d->client,cw-agdp()*minpadding*5,agdp()*minpadding,agdp()*minpadding,text,acfg()->textfg,isbig);
...
ACONTROLP actext(
  ...
  int ch            = 0;
  if (text!=NULL)
    ch = ag_txtheight(cw-agdp()*minpadding*3,text,isbig)+(agdp()*(minpadding*3));
  else
    ch = h;
  ...
  ag_rect(&d->control_focused, 0, 0, w, h, acfg()->titlefg);
  if (text!=NULL)
    ag_text(&d->client,cw-agdp()*minpadding*3,agdp()*minpadding,agdp()*minpadding,text,acfg()->textfg,isbig);
