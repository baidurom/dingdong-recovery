...
void acfg_reset_text(){
  ...
  snprintf(acfg_var.text_menumain,64,"< Back");
  snprintf(acfg_var.text_menuback,64,"Main >");
...
void acfg_init_ex(byte themeonly){
  ...
  acfg_var.battlow      = ag_rgb(0xee,0x2c,0x2c);
  acfg_var.wintitlebg 	= ag_rgb(0x00,0x00,0x00);
  acfg_var.wintitlefg   = ag_rgb(0x00,0x00,0x00);
  acfg_var.winbarbg     = ag_rgb(0xFF,0xFF,0xFF);
  ...
  snprintf(acfg_var.rec_name,128,DD_NAME);
  snprintf(acfg_var.rec_version,128,DD_VERSION);
  snprintf(acfg_var.rec_iteration,128,DD_ITERATION);
  snprintf(acfg_var.rec_author,128,DD_BUILD_A);
  snprintf(acfg_var.rec_device,128,"Not Defined");
  snprintf(acfg_var.rec_date,128,DD_BUILD);
...
/***************************[ THEME ]**************************/
static char theme_name[DD_THEME_CNT][27]={
  "img.background",
  "img.titlebar",
  "img.navbar",
  "img.dialog",
  ...
  "img.radio.on.focus",
  "img.radio.on.push",
  "img.homebanner.background",
  "img.subbanner.background",
  "img.background.other"
};
...
char * atheme_key(int id){
  if (id<0) return NULL;
  if (id>=DD_THEME_CNT) return NULL;
...
dword aw_dispatch(AWINDOWP win){
  ...
  while(1){
    ...
    //printf("aw_dispatch get action: %d, atev.x: %d, atev.y: %d, atev.d: %d, atev.k: %d\n", action, atev.x, atev.y,atev.d,atev.k);
    ...
    switch (action){
      ...
      break;
      case ATEV_BACK:case ATEV_LEFT:{
        if (atev.k != TOUCH_CODE && atev.d){
          vibrate(30);
          msg = aw_msg(8,1,0,0);
        }
      }
    ...
    //printf("A: aw_dispatch send msg: %d, aw_gm(msg): %d, aw_gd(msg): %d\n", msg, aw_gm(msg), aw_gd(msg));
    if (aw_gm(msg)!=0) return msg;
  }
  //printf("B: aw_dispatch send msg: %d\n", msg);
  return msg;
}
...
void aw_alert(AWINDOWP parent,char * titlev,char * textv,char * img,char * ok_text){
  ...
  int txtW    = winW-(pad*4);
  int txtX    = winX + pad*2;
  int btnH    = agdp()*24;  //button heigth
  int titW    = ag_txtwidth(title,2);  //title width
  int titH    = ag_fontheight(2) + (pad*2); //title heigth
  
  PNGCANVASP winp = atheme("img.dialog"); //dialog window background picture
  PNGCANVASP titp = atheme("img.dialog.titlebar"); //title bar background picture
  APNG9      winv;
  APNG9      titv;
  int vtitY   = -1;
  int vpadB   = -1;
  int vimgX   = pad*2;
  if (titp!=NULL){
    if (apng9_calc(titp,&titv,0)){
      titH   += titv.t+titv.b;
      vtitY   = titv.t;
    }
  }
  if (winp!=NULL){
    if (apng9_calc(winp,&winv,0)){
      txtW   -= winv.l+winv.r;
      txtX    = winX + pad*2  + (winv.l);
      vimgX   = winX + pad*2  + (winv.l);
      vpadB   = winv.b;
    }
  }
  ...
  //-- Load Icon
  PNGCANVAS ap;
  byte imgE = 0; int imgW = 0; int imgH = 0; int imgA = 0;
  if(img) {
	  if (apng_load(&ap,img)){
	    imgE      = 1;
  ...
  //-- Calculate Button Size & Position
  int btnW    = winW/2 - pad*3;
  int btnY    = infY+infH+pad*2;
  int btnX    = (winW - btnW)/2 + winX;
  ...
  ag_textf(&alertbg,titW,titX+1,titY+1,title,acfg_var.dlgtitlebg_g,2);
  ag_text(&alertbg,titW,titX,titY,title,acfg_var.dlgtitlefg,2);
  ...
  //-- Draw Text
  ag_textf(&alertbg,txtW,txtX+1,txtY+1,text,acfg_var.dialogbg,1);
  ag_text(&alertbg,txtW,txtX,txtY,text,acfg_var.dialogfg,1);
  
  AWINDOWP hWin   = aw(&alertbg);
  acbutton(hWin,btnX,btnY,btnW,btnH,(ok_text==NULL?acfg_var.text_ok:ok_text),1,5);
  aw_show(hWin);
  byte ondispatch = 1;
  while(ondispatch){
    dword msg=aw_dispatch(hWin);
...
byte aw_confirm(AWINDOWP parent, char * titlev,char * textv,char * img,char * yes_text,char * no_text){
  ...
  acbutton(hWin,btnX,btnY,btnW,btnH,(yes_text==NULL?acfg_var.text_yes:yes_text),1,6);
  acbutton(hWin,btnX2,btnY,btnW,btnH,(no_text==NULL?acfg_var.text_no:no_text),1,5);
