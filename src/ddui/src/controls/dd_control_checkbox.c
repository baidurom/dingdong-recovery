 ...
 * Descriptions:
 * -------------
 * DD UI: Checkbox List Window Control
 *
 */
#include "../dd_inter.h"
...
void accheck_redrawitem(ACONTROLP ctl, int index){
  ...
  if (p->isTitle){
    ...
    ag_textf(c,d->clientTextW+(agdp()*14),(d->clientTextX-(agdp()*14))+1,p->y+p->ty,p->title,acfg()->titlebg_g,1);
    ag_text(c,d->clientTextW+(agdp()*14),d->clientTextX-(agdp()*14),p->y+p->ty-1,p->title,acfg()->titlefg,1);
  }
  else{
...
byte accheck_addgroup(ACONTROLP ctl,char * title, char * desc){
  ...
  snprintf(newip->title,64,"%s",title);
  snprintf(newip->desc,128,"%s",desc);
  newip->th       = ag_txtheight(d->clientTextW+(agdp()*14),newip->title,1);
  newip->dh       = 0;
  newip->ty       = agdp()*3;
...
void accheck_ondraw(void * x){
  ...
  if (d->focused){
    ...
    ag_draw_ex(pc,&d->client,ctl->x+agdp(),ctl->y,0,d->scrollY+agdp(),ctl->w-agdp()*2,ctl->h);
  }
  else{
