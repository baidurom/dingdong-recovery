/*
 * Copyright (C) 2011-2012 Micode ( http://Micode.net/ )
 *
...
static byte aipInitEventDev(AIP_EVP e){
  ...
  dd_debug("EV ST: minX: %d  maxX: %d  minY: %d  maxY: %d\n", e->p.xi.minimum, e->p.xi.maximum, e->p.yi.minimum, e->p.yi.maximum);
  dd_debug("EV MT: minX: %d  maxX: %d  minY: %d  maxY: %d\n", e->mt_p.xi.minimum, e->mt_p.xi.maximum, e->mt_p.yi.minimum, e->mt_p.yi.maximum);
...
static byte aipTranslateEvent(AIP_EVP e, struct input_event * ev){
  ...
  dd_debug("ev->type: %d, ev->code: %d, ev->value: %d\n", ev->type, ev->code, ev->value);
