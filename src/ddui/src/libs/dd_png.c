 ...
 * Descriptions:
 * -------------
 * DD Installer PNG & PNG Font Handler
 *
 */
#include <png.h>
#include "../dd_inter.h"
...
byte apng_load(PNGCANVAS * pngcanvas,char* imgname) {
  ...
  if (imgname[0]=='@'){
      ...
      icotheme_name++;
      snprintf(zpath, 255, "%s/icons/%s.png",DD_DIR,icotheme_name);
  }
  else
      snprintf(zpath, 255, "%s/%s.png",DD_DIR,imgname);
...
byte apng_loadfont(PNGFONTS * pngfont,const char* imgname) {
  ...
  //-- LOAD DATA FROM ZIP
  char zpath[256];
  snprintf(zpath, sizeof(zpath)-1, "%s/%s.png",DD_DIR,imgname);
  AZMEM data_png;
  if (!az_readmem(&data_png,zpath,1)) return 0;
