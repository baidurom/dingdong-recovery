 ...
 *para:name->the file name ,execute path, no prefix
 *ins: "input0"
...
{
    dd_debug("[%s]ev->type:%4x , ev->code:%4x, ev->value:%4x\n",str,  ev->type, ev->code, ev->value);
}
...
int atouch_wait_ex(ATEV *atev, byte calibratingtouch){
  ...
  while (1){
    ...
    else if ((key!=0)&&(key==acfg()->ckey_menu))    return ATEV_SELECT;
    else if ((key!=0)&&(key==acfg()->ckey_home))    return ATEV_HOME;
    else{
      /* DEFINED KEYS */
      switch (key){
        /* RIGHT */
