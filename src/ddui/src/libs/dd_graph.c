...
static void print_screeninfo(int n)
{
	printf("\n-----%d: get screen info-----\n", n);
	printf("ag_fbv.red.offset: %d\n", ag_fbv.red.offset);
	printf("ag_fbv.red.length: %d\n", ag_fbv.red.length);
	printf("ag_fbv.green.offset: %d\n", ag_fbv.green.offset);
	printf("ag_fbv.green.length: %d\n", ag_fbv.green.length);
	printf("ag_fbv.blue.offset: %d\n", ag_fbv.blue.offset);
	printf("ag_fbv.blue.length: %d\n", ag_fbv.blue.length);
	printf("ag_fbv.transp.offset: %d\n", ag_fbv.transp.offset);
	printf("ag_fbv.transp.length: %d\n", ag_fbv.transp.length);
...
byte ag_init(){
  ...
  //-- Open Framebuffer
  ag_fb = open(DD_FRAMEBUFFER, O_RDWR, 0);
  ...
  dd_debug("open framebuffer failed\n");
  return 0;
}
...
//-- Sync Display
void ag_copybusy(char * wait_text){
  CANVAS tmpc;
  ...
  char * wait = "loading...";
  int pad     = agdp()*50;
  int txtW    = ag_txtwidth(wait,1);
  int txtH    = ag_fontheight(1);
  int txtX    = (agw()/2)-(txtW/2);
  int txtY    = agh() - agdp()*30 -(agdp()*4) - txtH;
  int winH    = txtH+(pad*2);
...
/*  int i;
  ...
  */
  ag_text(&tmpc,txtW,txtX,txtY,wait,0xffff,1);
  ag_oncopybusy=0;
  
  int bs_x = (agw()/2) - (ag_busywinW/2);
  int bs_y = agh() - agdp()*30;
  int bs_h = agdp()*2;
...
byte ag_rect_block(CANVAS *_b,int x, int y, int w, int h, color cl){
  if (_b==NULL) _b=&ag_c;
...
byte ag_fontready(byte isbig){
  ...
  byte isfreetype;
  switch(isbig) {
	case 0:  isfreetype = AG_SMALL_FONT_FT;  break;
	case 1:  isfreetype = AG_MEDIUM_FONT_FT; break;
	case 2:  isfreetype = AG_BIG_FONT_FT;    break;
	default: isfreetype = AG_MEDIUM_FONT_FT; break;
  }
  if (isfreetype) return aft_fontready(isbig);
  PNGFONTS * fnt;
  switch(isbig) {
	case 0:  fnt = &AG_SMALL_FONT;  break;
	case 1:  fnt = &AG_MEDIUM_FONT; break;
	case 2:  fnt = &AG_BIG_FONT;    break;
	default: fnt = &AG_MEDIUM_FONT; break;
  }
  return fnt->loaded;
}
...
byte ag_loadmediumfont(char * fontname, byte is_freetype, char * relativeto){
  while (ag_oncopybusy) usleep(50);
  ag_font_onload=1;
  byte r=0;
  if ((is_freetype!=0)&&(relativeto!=NULL)){
    ...
    if (r) AG_MEDIUM_FONT_FT = 1;
  }
  else{
    apng_closefont(&AG_MEDIUM_FONT);
    r=apng_loadfont(&AG_MEDIUM_FONT,fontname);
    if (r) AG_MEDIUM_FONT_FT = 0;
  }
  ag_font_onload=0;
  return r;
}
...
color ag_getcolorset(int color_index){
  ...
  switch(color_index){
    ...
    case 28: cl=acfg()->battlow; break;
    case 29: cl=acfg()->wintitlebg; break;
    case 30: cl=acfg()->wintitlefg; break;
    case 31: cl=acfg()->winbarbg; break;
  };
  return cl;
}
