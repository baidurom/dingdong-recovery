...
#define VERSION_FILE "/data/.layout_version"
void setup_data_media() {
    ...
    for (i = 0; i < num_volumes; i++) {
        ...
        if (strcmp(vol->fs_type, "datamedia") == 0) {
            ...
            if((fp != NULL) && (lstat("/data/media/0", &st) == 0)){
                ensure_path_mounted(VERSION_FILE);
                char version_path[2];
                version_path[1] = 0;
                fread(version_path, 1, 1, fp);
                fclose(fp);
                if(strncmp(version_path, "2", 1) == 0){
        ...
        if (strcmp(vol->fs_type, "datashare") == 0) {
            struct stat st;
            FILE* fp = fopen(VERSION_FILE, "rb");
            if((fp != NULL) && (lstat("/data/share/0", &st) == 0)){
                ensure_path_mounted(VERSION_FILE);
                char version_path[2];
                version_path[1] = 0;
                fread(version_path, 1, 1, fp);
                fclose(fp);
                if(strncmp(version_path, "2", 1) == 0){
                    LOGI("Using /data/share/0 for %s.\n", vol->mount_point);
                    rmdir(vol->mount_point);
                    mkdir("/data/share/0", 0755);
                    symlink("/data/share/0", vol->mount_point);
                    return;
                }
            }
            LOGI("Using /data/share for %s.\n", vol->mount_point);
            rmdir(vol->mount_point);
...
int format_device(const char *device, const char *path, const char *fs_type) {
    ...
    if (strcmp(fs_type, "ext4") == 0) {
        ...
        reset_ext4fs_info();
        int result = make_ext4fs(v->device, v->length, v, NULL);
        if (result != 0) {
            LOGE("format_volume: make_extf4fs failed on %s\n", device);
            return -1;
        }
...
int format_unknown_device(const char *device, const char* path, const char *fs_type)
{
    ...
    if (strcmp(path, "/data") == 0) {
        sprintf(tmp, "cd /data ; for f in $(ls -a | grep -Ev \"media|share|\.layout_version\"); do rm -rf $f; done");
        __system(tmp);
    }
    else {
